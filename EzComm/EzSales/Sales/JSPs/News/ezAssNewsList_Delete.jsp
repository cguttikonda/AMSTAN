...
%>
			<tr>
			<td width=5%><a href="javascript:delNews('<%=newsId%>')"><img style="valign:bottom" src="../../Images/Common/delete_icon.gif" height="20" width="20" border=0  style='cursor:hand' title="Delete News"></a></td>
			<td><h3 ><a href="javascript:ezViewNews('<%=tempText%>','<%=newsSub%>','<%=newsId%>','<%=newsSyskey%>','<%=readFlag%>','<%=newsType%>','<%=newsType_F%>','<%=readStat%>','<%=readDate%>','<%=ackStat%>','<%=createdOn%>','<%=newsCat%>')" style="color:<%=fontColor%>">
