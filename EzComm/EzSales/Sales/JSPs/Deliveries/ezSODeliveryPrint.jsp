 ...
 <%
 	...
 	String dlvNo = request.getParameter("delivNo"); //request.getParameter("custInvNo");	//0090075491
 	...
 	//response.setHeader ("Content-Disposition", "attachment;filename="+dlvNo+".pdf");
 	...
 	try{
 		 if ("PDF".equals(type))
 		 {
			 ...
			 try
			 {
				 ...
				 printDetail.setValue("V2","OBJTYPE");
				 printDetail.setValue("ZGRV","MSGTYPE");
				 //printDetail.setValue("ZASE","MSGTYPE");
				 printDetail.setValue(zeroPad(soldTo,10),"CUSTOMER");
				 printDetail.setValue("P","DOCTYPE");
				 
				 printDetail.setValue(zeroPad(dlvNo,10),"OBJNUMBER");
				...
				try{
					...
					sos.write(Z_STRING);
					ezc.ezcommon.EzLog4j.log("============>1234--> send o/p Stream for Delivery output for Delivery nr "+dlvNo,"I"); 
					}
				}catch(Exception err){
 ...
 %>	        
  ...
  <form id="myForm" name="myForm" method="POST">
	...
	<input type="hidden" name="dlvNumber" value="<%=dlvNo%>">
	<input type="hidden" name="billHeadersCnt" value="1">
 </form>
 <div id="dialog-confirm" title="Print Error">
	<p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>Could not Print at this time. Click to go back to Delivery Document</p>
</div>		
</body>
 <%
 	}
