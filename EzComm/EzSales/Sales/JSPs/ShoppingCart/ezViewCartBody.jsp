...
<!-- Start of the Styles and Scripts of ezViewCartBody.jsp -->
<link type="text/css" rel="stylesheet" href="../../Library/Styles/Cart/ezViewCartBody.css" />
<script type="text/javascript" src="../../Library/JavaScript/Cart/ezCartAlerts.js"></script>
<!--<script type="text/javascript" src="../../Library/JavaScript/Cart/ezViewCartBody.js"></script>-->
<!-- End of the Styles and Scripts of ezViewCartBody.jsp -->
...
function prepareOrder()
{
	...
	for (i=0; i<cartRows; i++)
	{
		...
		var prdBrnd = eval("document.myForm.brandName_"+[i]).value;
		var prdDiv = eval("document.myForm.prodDiv_"+[i]).value;

		if (prdDiv == "D9")
		{
			dxvCR = dxvCR - 1;
		}
		if (prdBrnd == "DXV" && prdDiv != "D9")
		{
			dxvCount = dxvCount + 1;
		}
	}
	...
	if (qsCount != cartRows && qsCount != 0)
	{
  	...
  	if (dxvCount != dxvCR && dxvCount != 0)
  	{
		new HelloBar( '<span> You cannot mix DXV Products with Other Products. Please fix.</span>', {
			showWait: 100,
			positioning: 'sticky',
			fonts: 'Arial, Helvetica, sans-serif',
			forgetful: true,
...
%>
		...
		var Y = 'Y';
		var N = 'N';
		content.innerHTML = '<center><div id="main_content"><h2><font size=4px align=center>Issue with item(s) found while adding to cart.<br>Please </span><a href="JavaScript:downloadErr(); closeWin();"><font color=red>Click Here</font></a><font> to download items with discrepancies to MS Excel.</font><br><input type="button" value="close" onclick="closeWin()"><center>';
		// Add these elements to the body

		document.body.appendChild(darkbox);
		document.body.appendChild(content);

		// Calculate coordinates and such
		var pos_top = document.documentElement.scrollTop
		var pos_left = document.documentElement.scrollLeft;
		var screen_width = document.documentElement.clientWidth;
		var screen_height = document.documentElement.clientHeight;
...
<%
	...
	if(catType_C == null || catType_C.equals("null")) catType_C = "";
...
<div class="col1-set">
<div class="info-box">
<strong>Enter Material Code and Quantity to Quickly Add to Cart/Order
<!--<br>Mixing Quick Ship Orders with Non Quick Ship will lead to Processing Delays and violation of Quick Ship Program-->
</strong>
</br>
</div> <!-- info-box-->
...
%>
	...
	<div id="ajaxid" style="width: 980px; height:320px; display: none; overflow-x: hidden">
	...
	<div id="ajaxidAll" style="width: 1000px; height:500px; display: none; overflow-x: hidden">
		<div align=center  style="padding-top:10px;">
		<ul>
			<li>&nbsp;</li>
			<li><img src="../../Library/images/loading.gif" width="100" height="100" alt=""></li>
	...
	<td>
		<br><button type="button" class="button" value="Add To Cart" title="Add To Cart" onclick="javascript:addToCartFav('C','C','C')">
...
%>
			<li>
				<div id='uploadUHDiv' style="display:block;float: left;">
				<a href="#uploadAtt" class="fancybox"><span>Import From Excel</span></a>
				</div>
			</li>
			<li>
				<div id='uploadSpreadJS' style="display:block;float: left;">
				<a href="#uploadSJS" class="fancybox"><span>Multiple Line Entry</span></a>
				</div>
			</li>
			</ul>
		</li>
	...
	<div id="uploadSJS" style="width:100%; display:none">
		<iframe  width="850px" height="350px" scrolling=no src='ezUploadSpreadJS.jsp'></iframe>
	</div>
...
<div id="boxes">
	<div id="dialog" class="window"><a href="#" class="close"/><font color=white>[x] Close</font></a></div>
	<div id="mask"></div>
</div>
<div class="col1-set">
<div class="info-box">
...
<%
		...
		if(prepShipTo!=null && !"".equals(prepShipTo) && !"null".equals(prepShipTo))
		{
			...
			custAttrsHT = getCustAttrs(prepShipTo,Session);
		}
		...
		for(int i=0;i<cartRows;i++)
		{
			...
			String brandName   = (String)Cart.getDiscPer(i);
			String imgLink     = (String)Cart.getCurrency(i);
			String poProgType  = (String)Cart.getVarPriceFlag(i);
			...
			if("N/A".equals(prodStat_SS)) prodStat_SS = "";
			...
			if(listPrice_S==null || "null".equals(listPrice_S) || "".equals(listPrice_S)) listPrice_S="0.00";
			if(eanUpc_S==null || "null".equals(eanUpc_S)) eanUpc_S="";	
			if(ordQty_S==null || "null".equals(ordQty_S)) ordQty_S="0";	 
			if(jobQunTot_S==null || "null".equals(jobQunTot_S)) jobQunTot_S ="0";
			...
			if(!"2".equals(userType_A) || (prepShipTo!=null && !"".equals(prepShipTo) && !"null".equals(prepShipTo)))
			{
				try
				{
					String custAttr	= (String)custAttrsHT.get(salesOrg_S);
			...
			checkOne = "";
			if(dispProd_S!=null && !"null".equals(dispProd_S) && !"".equals(dispProd_S)) 
			{
			...
			int cartQtys	= bquoteQty.intValue();
...
%>
			...
			<input type="hidden" name="brandName_<%=i%>" value="<%=brandName%>">
			<input type="hidden" name="prodAttr_<%=i%>" value="<%=prodAttr_S%>">
...
%>
				...
				<p class="cat-num" style="color:red">Not Allowed in Your Portfolio or Default Ship-To</p>
<%
			}
...
%>
			...
			<td width=18% class="a-center">
				<input name="cartQty_<%=i%>" id="cartQty_<%=i%>" value="<%=cartQtys%>" size=10 class="input-text-qty" maxlength="9"  onkeypress="return alpha(event)" onblur="quoteQtyChk('<%=i%>')" onkeyup="hideBtn()"></input>
...
<%
			if(cat_Ar.equals(catalog_S) && ent==0)
			{
				...
				if("DXV Chinaware".equals(catalog_S) || "DXV Faucets".equals(catalog_S) || "DXV Furniture".equals(catalog_S) ||
				   "DXV Tubs".equals(catalog_S) || "DXV Sinks (Non-CW)".equals(catalog_S) || "DXV Repair Parts".equals(catalog_S))
				{
%>
				<span id='remId'><%=catalog_S%></span>
<%
				}
				else
				{
