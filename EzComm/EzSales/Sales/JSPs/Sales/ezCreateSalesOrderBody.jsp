...
function calNetPrice(ind,multVal)
{
	...
	var qtyVal = $(document.getElementById("tqty_"+ind)).val();
	var netHVal = $(document.getElementById("netPriceHid_"+ind)).val();
	var shipHandVal = $(document.getElementById("freightTotal")).val();	
	...
	if(!isNaN(len))
	{
		...
		for(i=0;i<len;i++)
		{
			...
			if(len==1)
				lNo = document.generalForm.lineItem.value;
			else
				lNo = document.generalForm.lineItem[i].value;

			grandTotal += (parseFloat($(document.getElementById("netPrice_"+lNo)).val())*parseFloat($(document.getElementById("tqty_"+lNo)).val()));
		}
	}
	...
	$(document.getElementById("subTotalItem_"+ind)).html("$"+subVal);
	$(document.getElementById("subTotalAll")).html(grandTotal);
	$(document.getElementById("gTotal")).html(finalTotal);
}
function calMulti(multiObj,ind)
{
	var multVal = multiObj.value;
	var lpVar = "listPrice_"+ind+"";
	var lP = $(document.getElementById(lpVar)).val();

	var netVal = (parseFloat(lP)*parseFloat(multVal)).toFixed(2);
	if(netVal!='')
	{
		$(document.getElementById("netPrice_"+ind)).val(netVal);
		$(document.getElementById("dispNetPrice_"+ind)).html(netVal);
...
<%
	...
	boolean focOrderByInt = false;

	if("FOC".equals(catType_C) && ("Y".equals(focApprover) || loginUserId.equals(approver)))
	{
		focOrderByInt = true;
	}
...
<input type="hidden" name="expEDDFlag" value="<%=expEDDFlag%>">
...
<%
	...
	Hashtable totNetValCatHT = new Hashtable();
	...
	for(int ar=0;ar<catalogList.size();ar++)
	{
		...
		for(int h=0;h<rCount;h++)
		{
			...
			String itemDistChnl	= distCh;	//viewCart.getDistChnl(h);
			...
			if(dispOrderByInt) stdMultiChng_I = "Y";
			...
			for(int j=0;j<retCond.getRowCount();j++)
			{
				...
				if(lineNo.equals(condLineNo) && splitKey.equals(splitKeyChk))
				{
					...
					if("ZFOC".equals(condType))
					{
						price_ZFOC = retCond.getFieldValueString(j,"Condvalue");

						try
						{
							price_ZFOC = new java.math.BigDecimal(price_ZFOC).setScale(2,java.math.BigDecimal.ROUND_HALF_UP).toString();

							if(focOrderByInt)
							{
								itemValue = (new java.math.BigDecimal(price_ZFOC).setScale(2,java.math.BigDecimal.ROUND_HALF_UP)).toString();
				...
				if(splitKey.equals(splitKeyChk) && lineNo.equals(hgLvItem))
				{
					...
					for(int j=0;j<retCond.getRowCount();j++)
					{
						...
						if(lineNo_B.equals(condLineNo) && splitKey.equals(splitKeyChk_B))
						{
							...
							if("ZFOC".equals(condType_B))
							{
								try
								{
									priceB_ZFOC = ((new java.math.BigDecimal(priceB_ZFOC)).add((new java.math.BigDecimal(retCond.getFieldValueString(j,"Condvalue"))))).setScale(2,java.math.BigDecimal.ROUND_HALF_UP).toString();
			...
			if(!"0".equals(priceB_ZFOC)) price_ZFOC = priceB_ZFOC;
...
<%
				...
				{
%>
					<br><strong><font color=red>Alternative Product:</font></strong><br><a href="javascript:getProductDetails('<%=matChnge%>')" title="<%=matChnge%>"><%=matChnge%></a>
<%
					if(!matRep)
					{
						matRep = true;
...
%>
				...
				<td class="price">$<%=listPrice%>
				<input type="hidden" name="listPrice_<%=lineItem%>" id="listPrice_<%=lineItem%>" value="<%=listPrice%>">
				</td>
				<td class="price">
<%
				if(focOrder)//!"CU".equals(userRole)
...
%>
								...
								<select name="stdMulti_<%=lineItem%>" id="stdMulti_<%=lineItem%>" onChange="calMulti(this,<%=lineItem%>)">
<%
								String multiArray[] = new String[]{"0.10","0.23","0.25","0.35"};

								for(int al=0;al<multiArray.length;al++)
								{
									String mulVal = multiArray[al];
									String selected = "";

									if(stdMultiplier.equals(mulVal)) selected = "selected";
%>
									<option value='<%=mulVal%>' <%=selected%>><%=mulVal%></option>
<%
								}
%>
								</select>
...
<%
				if(focOrder)//!"CU".equals(userRole)
				{
					...
					{
%>
					<input type="hidden" name="netPrice_<%=lineItem%>" id="netPrice_<%=lineItem%>" value="<%=price_N%>"><span id="dispNetPrice_<%=lineItem%>"><%=price_N%></span>
<%
					}
					else
					{
						if("FOC".equals(catType_C) || dispOrderByInt)
						{
							...
							{
%>
								<input type="hidden" name="netPrice_<%=lineItem%>" id="netPrice_<%=lineItem%>" value="0"><%=price_ZFOC%>
<%
							}
							else
							{
%>
								<input type="hidden" name="netPrice_<%=lineItem%>" id="netPrice_<%=lineItem%>" value="0"><%=price_N%>
<%
							}
						}
						else
...
%>
					<input type="hidden" name="listPriceF_<%=lineItem%>" value="<%=price_ZFOC%>">
					...
					<input type="hidden" name="product" value="<%=prodNoUD%>" class="productForLen">
...
%>			
			<td class="price"><%=tqty%>
			<input type="hidden" name="tqty_<%=lineItem%>" id="tqty_<%=lineItem%>" value="<%=tqty%>">
			</td>
			<td class="price">
<%
...
%>
			...
			<td class="price"><span id="subTotalItem_<%=lineItem%>">$<%=checkAuth(tItemVal,"VIEW_PRICES",userAuth_R)%></span></td>
...
<%
			...
			if(cat_Ar.equals(itemVenCatalog))// && ent==0)
			{
				...
				if("Repair Parts".equals(itemVenCatalog) || "DXV Chinaware".equals(itemVenCatalog) || "DXV Faucets".equals(itemVenCatalog) ||
				   "DXV Furniture".equals(itemVenCatalog) || "DXV Tubs".equals(itemVenCatalog) || 
				   "DXV Sinks (Non-CW)".equals(itemVenCatalog) || "DXV Repair Parts".equals(itemVenCatalog))
				{
					pntspcs="$";

					try
					{
						java.math.BigDecimal totNetValBD = new java.math.BigDecimal("0");
						if(totNetValCatHT.containsKey(itemVenCatalog))
							totNetValBD = new java.math.BigDecimal((String)totNetValCatHT.get(itemVenCatalog));

						totNetValBD = totNetValBD.add(new java.math.BigDecimal(itemValue));

						totNetValCatHT.put(itemVenCatalog,totNetValBD.toString());
					}
					catch(Exception e){}
				}
			}
			...
			{
%>
				<td id="pointsgroup"><%=itemVenCatalog%></td>	<!--:<%=pntspcs%><%=(String)totNetValCatHT.get(itemVenCatalog)%>-->
<%
			}
			else
			{
...
%>
	...
	<tr>
		...
		<td class="price">$<span id="subTotalAll"><%=checkAuth(tTotItemVal,"VIEW_PRICES",userAuth_R)%></span></td>
	</tr>
	<tr>
...
%>
	<tr>
		...
		<td class="price">$<span id="gTotal"><%=checkAuth(tGTotVal,"VIEW_PRICES",userAuth_R)%></span></td>
	</tr>
	<!--<tr>
...
<input type="hidden" name="freightTotal" id="freightTotal" value="<%=freightHD00%>">
