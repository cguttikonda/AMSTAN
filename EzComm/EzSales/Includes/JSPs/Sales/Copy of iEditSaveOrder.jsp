		...
		if("TRANSFERED".equalsIgnoreCase(status))
		{
			...
			if(retNeg!=null && retNeg.getRowCount()>0)
			{
				...
				miscParamsNeg.setIdenKey("MISC_UPDATE");
 				miscParamsNeg.setQuery("UPDATE EZC_ORDER_NEGOTIATE SET EON_MODIFIED_BY='"+Session.getUserId()+"', EON_STATUS='FOCACCEPTED' WHERE EON_ORDER_NO='"+webOrdNo_S+"'");
 				negParams.setLocalStore("Y");
				negParams.setObject(miscParamsNeg);
				Session.prepareParams(negParams);				
				//log4j.log("<<<<setOrderNo>>>>>"+ezOrderNegotiateParams.getOrderNo(),"I");
...
<%
	...
	if("NEW".equalsIgnoreCase(status))
	{
		if(SAPnumber)
		{
			...
			sendMail = true;
			offLink = "http://"+request.getServerName()+"/AST/EzComm/EzSales/Sales/JSPs/Sales/ezOfflineLogin.jsp?webOrNo="+weborno+"&soldTo="+mySoldTo_A+"&sysKey="+sysKey+"&status="+status;
