...
<%
	...
	if(status_Ord!=null && !"null".equalsIgnoreCase(status_Ord))
	{
		...
		miscParamsOrdChk.setQuery("SELECT ESDH_SYS_KEY,ESDH_DOC_NUMBER,ESDH_PO_NO,ESDH_STATUS FROM EZC_SALES_DOC_HEADER WHERE ESDH_DOC_NUMBER in ('"+soNum+"')");
		...
		if(retOrdChk!=null && retOrdChk.getRowCount()>0)
		{
			...
			salesAreaCode_E = retOrdChk.getFieldValueString(0,"ESDH_SYS_KEY");
			...
			if("SUBMITTED".equals(status_Ord))
			{
				...
				{
					actTaken = true;
					showMsg_A = "You are not authorized to view this order";
				}
				else if(ordHeadStatus!=null && !"SUBMITTED".equalsIgnoreCase(ordHeadStatus.trim()))
				{
					actTaken = true;
			...
			if("NEGOTIATED".equals(status_Ord))
			{
				...
				if(!soldTo_AL.contains(soldToChk))
				{
					actTaken = true;
					showMsg_A = "You are not authorized to view this order";
				}
				else if(ordHeadStatus!=null && !"NEGOTIATED".equalsIgnoreCase(ordHeadStatus.trim()) && "TRANSFERED".equalsIgnoreCase(ordHeadStatus.trim()))
				{
					actTaken = true;
...
<%
	...
	ezc.ezcommon.EzLog4j.log(">>>>>>>sdHeader>>>>>>>>>>>>>>>>>>>>>>"+sdHeader.toEzcString(),"I");
