<%
	...
	if("TRANSFERED".equals(status) && !userAuth_R.containsKey("SUBMIT_ORDER")) authCheck = false;
	else if("SUBMITTED".equals(status) && !userAuth_R.containsKey("FOC_ORDER")) authCheck = false;
	else if("NEGOTIATED".equals(status))
	{
		if(!userAuth_R.containsKey("SUBMIT_ORDER")) authCheck = false;
...
<%
	...
	log4j.log("****SAPnumber*****10","D");
			...
			rowItm.setSplFlag("");	// Need to set for products from specail catalogs like quick ship etc..
...
<%
			...
			{
				ReturnObjFromRetrieve partnersRet_C = null;
				...
				if(soldToCode!=null)
				{
					...
					EzcParams mainParams_C = new EzcParams(false);
					mainParams_C.setObject(adminUtilsParams);
					Session.prepareParams(mainParams_C);

					partnersRet_C = (ReturnObjFromRetrieve)AUM.getUsersByPartnerValueAndArea(mainParams_C);
...
//log4j.log("sysKey=======>"+sysKey+"=============>wfParticipant"+wfParticipant+"==============>desiredStep_mail"+desiredStep_mail+"=========>Template"+session.getValue("Template"),"W");
//log4j.log("retsoldto_Q=======>"+retsoldto_Q.toEzcString(),"W");
				...
				{
					sendToUser=sendToUser;//sendToUser = saveEmail;
				}
				else
				{
...
<%
	...
	if("NEW".equalsIgnoreCase(status))
	{
		if(SAPnumber)
		{
			...
			offLink = "http://"+request.getServerName()+"/AST/ezLinkController.jsp?link="+encrypText;
	...
	else if("NEGOTIATED".equalsIgnoreCase(status))
	{
		if(SAPnumber) 
		{
			...
			{
				msgSubject = "Purchase Order "+poNumber+" has been Approved";
				msg = "Order with PO No<font color=white>  "+poNumber+"</font>  has been Approved and sent for acceptance.";

				msgText = "Dear Concerned<br><br>Purchase Order created by  "+soldToName_N+"("+mySoldTo_A+") with reference no: <a href='"+offLink+"'>"+poNumber+"</a> has been Approved.<br>";
				msgText += "<br>";
				msgText += eMailData;
				msgText += "<br>";
	...
	else
	{
		if(sDocNumber == null || sDocNumber.trim().length()==0)
			...
			*/
			...
			if(SAPnumber)
			{
				toEncryp = "EzComm/EzSales/Sales/JSPs/Sales/ezSalesOrderDetails.jsp?salesOrder="+tempSDoc+"&soldTo="+mySoldTo_A+"&shipTo="+shipToCode+"&status="+status;
				log4j.log("toEncryptoEncryptoEncryp======>"+toEncryp, "D");
				encrypText = MD5(toEncryp);
				log4j.log("encrypTextencrypTextencrypText======>"+encrypText, "D");
				offLink = "http://"+request.getServerName()+"/AST/ezLinkController.jsp?link="+encrypText;
				...
				msg = "<h2>YOUR ORDER HAS BEEN RECEIVED AND SAVED WITH PO# <a href=\"javascript:getDetails(\'"+tempSDoc+"\',\'"+mySoldTo_A+"\')\">"+poNumber+"</a></h2>";
				...
				msg = msg+"<p> Click here to <a href=\"javascript:void(0)\">Print</a> the Submitted Order for your Records </p>";
				...
				{
					msgSubject = "ASB BP: PO Receipt For "+poNumber+", Ship-to Acc. "+shipToCode;
					msgText = "Dear Concerned<br><br>";
					msgText += "Thank you for your recent order to American Standard Brands. Enclosed you will find a purchase order link referencing the subject PO.";
					msgText += "Please go to www.MyASB2B.com or copy and paste the following link into your browser’s address bar to check the order status for recent updates/activities.";
					msgText += "<br><br><a href='"+offLink+"'>Click here to see the PO</a>";
					msgText += "<br><br>Please note the following:";
					msgText += "<br>&nbsp;&nbsp;&middot;&nbsp;&nbsp;PCO charges will be added during time of shipment.";
					msgText += "<br>&nbsp;&nbsp;&middot;&nbsp;&nbsp;Cancellation must be received 3 business days prior to the Expected Ship Date.";
					msgText += "<br>&nbsp;&nbsp;&middot;&nbsp;&nbsp;Orders for custom product cannot be cancelled.";
					msgText += "<br>&nbsp;&nbsp;&middot;&nbsp;&nbsp;Preferred browsers are Google Chrome, Fire Fox and IE 9 or higher.";
					msgText += "<br><br>Please do not reply to this system generated e-mail. To help ensure the highest levels of service, we ask that you direct all inquiries to our customer care department by calling our toll-free number at (800) 442-1902 for assistance.";
					//msgText += eMailData;
					//msgText += "<br>";
					//msgText += "<br>Regards,<br>"+Session.getUserId()+" "+custName_M;
					msgText += "<br><br>ASB Business Portal Team";
					msgText += "<br>American Standard Brands";
					msgText += "<br>1 Centennial Ave";
					msgText += "<br>Piscataway, NJ 08855";
					msgText += "<br>P: 1-800-442-1902";
				}
			}
			else
			{
