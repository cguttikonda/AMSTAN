...
<%
		...
		String colExSQ[]={"SALESDOC","ITEMNO","MATERIAL","MATDESC","VALIDFROM","VALIDTO","REQQTY","NETPRICE","NETVALUE","DOCDATE","SOLDTO","SOLDTONAME","POJOBNAME","DOCSTATUS","SHIP_TO","SHIP_TO_NAME"};
		...
		try
		{
			...
			if (openStat.equalsIgnoreCase("E") || openStat.equalsIgnoreCase("ES") )
			...
			{
				JCO.Table custSelectionTable = functionEx.getTableParameterList().getTable("CUSTOMER_SELECTION");
			...
			//out.println(" Status +++"+openStat+"+++"+DateFrom+"+++"+DateTo);
			try
			{
			...
			if(headerTable!=null)
			{
				...
				if (headerTable.getNumRows() > 0)
				{
					do
					{
						...
						retObjExSQ.setFieldValue("SHIP_TO",headerTable.getValue("SHIP_TO"));
						retObjExSQ.setFieldValue("SHIP_TO_NAME",headerTable.getValue("SHIP_TO_NAME"));
