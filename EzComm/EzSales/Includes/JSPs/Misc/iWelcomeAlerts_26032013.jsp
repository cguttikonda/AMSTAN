...
<%
	...
	String soldTos	 = "ENA_SOLDTO IN ('"+(String)Session.getUserId()+"','A','I'))";
	if("3".equals(userType_A))
		soldTos	 = "ENA_SOLDTO IN ('"+(String)Session.getUserId()+"','A'))";
...
<%
	...
	if("CU".equals(userRole))
	{
		...
		allSoldTo_W = (String)session.getValue("AgentCode");
	String canReqQry="SELECT COUNT(DISTINCT(ESCH_ID)) CANCELREQCNT,ESCH_STATUS STATUS FROM EZC_SO_CANCEL_HEADER,EZC_SO_CANCEL_ITEMS WHERE ESCH_ID=ESCI_ID AND ESCH_TYPE IN ('RC') AND ESCH_STATUS IN ("+cancelStat+")";//ESCH_CREATED_ON BETWEEN  convert(DATETIME,'03/01/2012',110)  and  convert(DATETIME,'12/31/2015',110) AND 
	if("CU".equals(userRole))
	...
	try
	{
		...
		if(canReqRet!=null && canReqRet.getRowCount()>0)
		{
			if("CU".equals(userRole)) 
			{
				int rowId = 0;
				try{
					rowId = canReqRet.getRowId("STATUS","A");
					if(rowId!=-1)
