...
headers[0] = new setData("MFGLIC","Manufacturing License")
headers[1] = new setData("FDA","FDA Document")
headers[2] = new setData("ISO","ISO Document")
headers[3] = new setData("WHO","WHO Document")
...
function changeValues(cd)
{
  	 ...
  	 if(cd=='MFGLIC')
  	 {
  	 	parentObj.serverMfgLic.value='¥'
  	 }
  	 else if(cd=='FDA')
  	 {
  	 	parentObj.serverFda.value='¥'
  	 }
  	 else if(cd=='ISO')
  	 {
  	 	parentObj.serverIso.value='¥'
  	 }
  	 else if(cd=='WHO')
  	 {
  	 	parentObj.serverWho.value='¥'
  	 }
}
...
<body SCROLL="NO" onLoad="setDefaults();showAttachments()" topBorder=0 onUnLoad="funUnLoad()">
...
for(var i=0;i<headers.length;i++)
{
	...
	document.write("<img src='../../../../EzCommon/Images/Body/attachfile.jpg' style='cursor:hand' border=none onclick=funAttach('"+i+"')>");
	
	document.write("</div><div id=remove"+i+" style='postion:absolute;visibility:hidden'>");
	document.write("<img src='../../../../EzCommon/Images/Body/remove.gif' style='cursor:hand' border=none onclick=removeFile('"+i+"','"+headers[i].code+"')>");
	...
	<!--<img src="../../Images/Buttons/<%=ButtonDir%>/done.gif" style="cursor:hand" border=none onClick="funClose()">
