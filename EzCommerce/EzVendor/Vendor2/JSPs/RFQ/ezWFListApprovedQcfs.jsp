...
%>
					...
					if(isNaN(len))
					{
						...
						{
							alert("PO cannot be created against this QCF as already contract has been created.");
							return false;
						}*/
					}
					else
...
%>		
			var dialogvalue=window.showModalDialog("ezSelectVendors.jsp?colNumbers="+collRFQNos,"","center=yes;dialogHeight=20;dialogWidth=50;help=no;titlebar=no;status=no;resizable=no")
			if(dialogvalue==undefined )
				return;
			else
				document.myForm.approvedVend.value=dialogvalue;
			hideButton();	
			document.myForm.action="ezConfirmPOCreateFromRFQ.jsp"
...
%>
					...
					if(isNaN(len))
					{
						...
						{
							alert("Contract cannot be created against this QCF as already PO has been created.");
							return false;
						}*/
					}
					else
...
%>		
				...
				if(dialogvalue==undefined )
					...
					document.myForm.approvedVend.value=dialogvalue;
				hideButton();	
				document.myForm.action="ezConfirmContractCreateFromRFQ.jsp";
		  		document.myForm.submit();
		  	}
		  	else
		  	{
...
function checkFields()
{
	...
	if(counter > 1)
	{
		alert("Please select Only 1 Collective RFQ No");
		return;
	}
...
function funOpenWin()
{
	...
	var retValue = window.showModalDialog("ezAddQcfQueriesWindow.jsp?COLNO="+num+"&DOCTYPE=QCF",window.self,"center=yes;dialogHeight=30;dialogWidth=50;help=no;titlebar=no;status=no;minimize:yes")	
	}
}
	...
	<Div id="theads">
		<Table  id="tabHead" width="96%" align=center border=0 borderColorDark=#ffffff borderColorLight=#006666 cellPadding=5 cellSpacing=1 >
			<Tr align="center" valign="middle">
				...
				<Th width="14%">Approved On</Th>
				<Th width="9%">Proposed Vendors</Th>
			</Tr>
		</Table>
	</Div>
...
%>   
		<tr>
			...
			<td width="12%" align=center><%=globalRet.getFieldValue(i,"RFQ_DATE")%></td>
			...
			<td width="15%"><%=myRet.getFieldValueString(i,"APPROVEDBY")%></td>
			<td width="14%" align=center><%=modifyDate(approvedDate)%></td>
			<td width="9%" align=center><a href ="javascript:showVendors('<%=myRet.getFieldValueString(i,"COLLETIVE_RFQ_NO")%>','R')"><%=myRet.getFieldValueString(i,"TOT_PROPOSED")%></a></td>
...
<%
	if(editable)
	{
			...
			if("MG".equals(chkUserRole) || "SM".equals(chkUserRole))
			{
              ...
              butActions.add("ProcessFun('PO')");
              butActions.add("ProcessFun('CONTRACT')");


	}
                butNames.add("&nbsp;&nbsp;Display/Print&nbsp;&nbsp;");  
                butNames.add("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Audit&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"); 
		butNames.add("&nbsp;&nbsp;Query&nbsp;&nbsp;");  
                butActions.add("openPrintWin()");
                butActions.add("showTrail()");
                butActions.add("funOpenWin()");
...
%>
  ...
  <input type="hidden" name="approvedVend" value="">
</form>
<Div id="MenuSol"></Div>
</body>
</html>
