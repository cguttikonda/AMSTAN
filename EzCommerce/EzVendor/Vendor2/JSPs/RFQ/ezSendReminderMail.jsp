...
<jsp:useBean id="PreProcManager" class="ezc.ezpreprocurement.client.EzPreProcurementManager" />
...
<%
	...
	String[] rfqNum = request.getParameterValues("RfqNo");
	
	String toInvite = "";
	if(request.getParameter("TOINVITE") != null)
		toInvite = request.getParameter("TOINVITE");
		
	String subStart = "";	
	String msgText = "";
	String rfqReturnType = "";
	if("Y".equals(toInvite))
	{
		subStart = "Invitation";
		rfqReturnType = "INVITE";
	}	
	else	
	{
		subStart = "Remainder";
	...
	String messageHeader = "This is a "+subStart+" For Quotation from Ranbaxy Laboratories Ltd.<BR><BR>";
	messageHeader += "Please click on the following link to View and Quote for the Material<BR><BR>";

		
	int loopLength = 0;
	if(soldTo != null)
	{
		loopLength = soldTo.length;
	}
	if(myChk != null)
	{
		loopLength = myChk.length;
	}
	

	for(int m=0;m<loopLength;m++)
	{
		String msgLink = "";
		msgText = "";
		msgSubject = subStart+" For Quotation From Ranbaxy Laboratories Ltd.";
		rfqNo  = rfqNum[m];
		vendor = soldTo[m];
		try
		{
		...
		//msgLink = "<Br><a href=\"http://"+mailIp+"/j2ee/EzCommerce/EzVendor/EzRanbaxyVendor/Offline/Quote/ezPrePostQuote.jsp?rfq="+rfqNo+"&user="+sendToUser+"&skey="+syskey+"\" target=\"_blank\">http://"+mailIp+"/EzCommerce/EzVendor/EzRanbaxyVendor/Offline/Quote/ezPrePostQuote.jsp?rfq="+rfqNo+"&user="+sendToUser+"&skey="+syskey+"</a>";
		//  msgLink = "<Br><a href=\"http://"+mailIp+"/EzCommerce/EzVendor/EzRanbaxyVendor/Offline/Quote/ezPrePostQuote.jsp?val="+encVal+"\" target=\"_blank\">http://"+mailIp+"/EzCommerce/EzVendor/EzRanbaxyVendor/Offline/Quote/ezPrePostQuote.jsp?rfq="+rfqNo+"&user="+sendToUser+"&skey="+syskey+"</a>";
		
		String toEncStr=rfqNo+sendToUser+syskey;
		String encId=ezEncrypt(toEncStr);
		//msgLink = msgLink+"<a href=\"http://"+mailIp+"/j2ee/EzCommerce/EzVendor/EzRanbaxyVendor/Offline/Quote/ezPrePostQuote.jsp?id="+encId+"\">http://"+request.getServerName()+"/j2ee/EzCommerce/EzVendor/EzRanbaxyVendor/Offline/Quote/ezPrePostQuote.jsp?id="+encId+"</a>";
		
		msgLink = msgLink+"<Br><a href=\"http://"+mailIp+"/j2ee/EzCommerce/EzVendor/EzRanbaxyVendor/Offline/JSPs/ezPrePostQuote.jsp?id="+encId+"\" target=_blank>Click here to post quotation.</a>";
		...
		for(int i=0;i<cnt;i++)
		{
			if("SMS".equals(reterpdef.getFieldValueString(i,"EUD_KEY")))
			{
		...
		msgText = messageHeader+"<BR>"+msgLink+"<BR><BR><BR>"+mailFooterText;
		
		System.out.println("msgTextmsgTextmsgTextmsgTextmsgTextmsgTextmsgTextmsgText:::"+msgText);
	...
	System.out.println("In external mail");
%>
...
<%     
		...
		System.out.println("$$$$$$$$$$$$$%%%%%%%%%%%%%%%%%%%%%%%From Send Remainder Start******************************");
		PreProcManager.ezUpdateRFQ(rfqezcparams);
		System.out.println("$$$$$$$$$$$$$%%%%%%%%%%%%%%%%%%%%%%%From Send Remainder End******************************");
	}   
	String retMessage = "A Mail has been sent Successfully for the Selected Vendors requesting for Quote.";
	response.sendRedirect("../Shipment/ezRfqMailMessage.jsp?Msg="+retMessage+"&RFQRETPAGE="+rfqReturnType+"&Frm="+request.getParameter("fromFrame"));
%>
<Div id="MenuSol"></Div>