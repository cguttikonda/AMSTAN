...
function showGraph(collRfq)
{
 ...
 function showDiv(n)
 {
         ...
         var num= document.myForm.divtabnum.value;
         if(n==0){
         ...
         for(var i=1;i<=2;i++)
 	 {
 	 	...
 	 	callflg=true;
			if(i==n)
			{
				...
				document.myForm.divtabnum.value=n;
				document.getElementById("tab"+i).style.visibility="visible"
				document.getElementById("tab"+i+"color").style.color="#000000"
				document.getElementById("tab"+i).style.width='95%'
				document.getElementById("tab"+i).style.height='55%'
			...
			else
			{
				...
				document.getElementById("tab"+i).style.height='0%'
...
<body onLoad="scrollMultiInit(5,1);scrollMultiInit(5,2);showDiv('0');" onResize="scrollMultiInit(5,1);scrollMultiInit(5,2);" onUnload="funUnload()" scroll=no>
...
%>
		<Div id="tab<%=m%>" style="overflow:auto;position:absolute;height:55%;width:95%;visibility:hidden">
<%
...
%>
			...
			<DIV id="theads<%=m%>">
				<table  id="tabHead<%=m%>" width="95%" align=center border=0 borderColorDark=#ffffff borderColorLight=#006666 cellPadding=5 cellSpacing=1 >
				<tr align="center" valign="middle">
				<th width="5%">&nbsp;</th>
				<th width="12%">Col.RFQ No</th>
				<th width="21%">Material</th>
				<th width="12%">Created Date</th>
				<th width="10%">Valid Upto</th>

				<th width="12%">Submited By</th>
...
%>
			...
			<DIV id="InnerBox<%=m%>Div" style="overflow:auto;position:absolute;width:95%;height:55%;<%=left%>">
...
%>
					...
					<Td <%=escRow%>  width="21%" ><input type=text <%=txtBg%> size=26 readonly value='<%=myRet.getFieldValueString(i,"MATERIAL_DESC")%>' title='<%=myRet.getFieldValueString(i,"MATERIAL_DESC")%>'></td>
					...
					<Td <%=escRow%>  width="12%" title='<%=myRet.getFieldValueString(i,"WFSTATUS")%> By <%=myRet.getFieldValueString(i,"SENTBY")%> On <%=modifyDate(statusDate)%>'>
...
<%
					if(m==1){
%>
						<Td <%=escRow%>  width="8%" align="center"><%=boldflg%><%=myRet.getFieldValueString(i,"TIMELEFT")%></Td>
						<Td <%=escRow%>  width="10%"><%=boldflg%><%=myRet.getFieldValueString(i,"REMARKS")%></td>
<%
					}else{
%>
						<Td <%=escRow%>  width="18%"><%=boldflg%><%=myRet.getFieldValueString(i,"REMARKS")%></td>						
<%
					}
%>
...
%>
	...
	<Div id="buttonDiv"  style="position:absolute;left:0%;width:100%;top:95%">
	<Table align=center>
	<Tr>
		<Td class=blankcell>
