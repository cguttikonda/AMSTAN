...
<%
	...
	ezc.ezpreprocurement.params.EziRFQHeaderParams rfqheaderparams = new ezc.ezpreprocurement.params.EziRFQHeaderParams();
	rfqheaderparams.setExt1("REPORT");
	rfqheaderparams.setCollectiveRFQNo(collRfq);
	EzcParams myParams = new EzcParams(false);
	myParams.setLocalStore("Y");
	myParams.setObject(rfqheaderparams);
	Session.prepareParams(myParams);
	ReturnObjFromRetrieve ret = (ReturnObjFromRetrieve)manager.ezGetRFQList(myParams);
...
this.rowValue=new Array();  
this.rowText=_rowText;
this.offset = 0;
return this;
}

function _rowText(rowValue)
{
...
{ 
	...
	//this.relativeValues[this.rows.length] = new Array();
...
function _rescaleGraph(g) 
{
	...
	//These steps added by srinivas k in order to change dynamically y-axis
	for(i=0;g.posMax % 5;i++)
	g.posMax++
	g.scale=Math.ceil(g.posMax/10)
	...
	for(var i = 0; i < g.rows.length; i++) 
	  {
	     for(var j = 0; j < g.rows[i].length; j++) 
	       {
	          rValue=Math.ceil( (g.rows[i][j]*100)/g.posMax)
	          g.relativeValues[rCount++]=rValue
...
function _writeLegendGraph() 
{
	...
	var st = "";
	st += "<TABLE border=0 cellSpacing=1 CELLPADDING=4><TR><TD><FONT FACE='Arial,Helvetica' SIZE=-1>";
	for(var i = 0; i < this.legends.length; i++) 
	{
		
		if(!this.legends[i]) continue;
		if(i >= this.rows.length) break;
		st += "<IMG SRC="+i+".gif border=0 HSPACE=3>"+this.legends[i]+"<BR>\n";
	}
	st += "</FONT></TD></TR></TABLE>";
	return st;
}
...
{
	var str = "";
	str += "<TABLE BORDER=0 CELLPADDING=0 cellSpacing=1 align=center>\n";
	...
	if(g.posMax > 0) 
	{
		...
		for(var j = 0; j < g.rows[0].length; j++) 
		{
			for(var i = 0; i < g.rows.length; i++) 
			{
				...
				if(parseFloat(g.rows[i][j])>parseFloat("0"))
				{
					str += "<table align=left><tr><td>"+g.rows[i][j]+"</td></tr><tr><td onClick=submitFun("+j+")><IMG BORDER=0 SRC=\"../"+1+".gif\" ";
					str += "ALT=\"";
					...
					if(g.dates) str +=g.dates[j];
						...
						str += "\" ";
					str += "WIDTH=30 ";
					str += "HEIGHT="+parseFloat(g.vscale*g.rows[i][j])+" ";
					str += " style=\"cursor:hand\"></td></tr></table>";
				...
				{
				       	str+="<FONT FACE='Arial,Helvetica' SIZE=-1 color='red'>"+g.rows[i][j]+"</font>";
				}	
...
function _writeXScaleGraph(g)
{
	...
	for(var i = 0; i < Math.floor(g.rows[0].length/g.skip); i++) 
	{
		...
		st += ">&nbsp;<FONT FACE='Arial,Helvetica' SIZE=1><B>";
		st += g.dates[i*g.skip] || "";
		st += "</B></FONT></TD>\n";
	}
...
function _writeScaleGraph(g, min, max) 
{
	...
	st += "<TD VALIGN=BOTTOM ROWSPAN=3 ALIGN=RIGHT>";
	var H = h - 3;
	for(var i = -1; i < n; i++) 
	{
	...
	//added to get the y axis
	st+= "</td><TD BGCOLOR=#000000 VALIGN=BOTTOM ROWSPAN=2 ALIGN=RIGHT>";
	st += "<IMG SRC=\"black.gif\" HEIGHT="+g.c+" WIDTH=1";
	st += "></td>";
...
<SCRIPT LANGUAGE="JavaScript1.2">
	...
	function submitFun(opt)
	{

		//document.myForm.poId.value=id[opt]
		//document.myForm.action="ezGraphByValue.jsp";
...
<%
	...
	{
%>
		val[<%=i%>]="<%=ret.getFieldValueString(i,"SOLD_TO")%>"
<%
		counters += "," + ret.getFieldValueString(i,"COUNTER");
	}
	counters = counters.substring(1);
%>
	

	//alert(val)
	// alert(id)
	//var xx = new Array(0,0,0,0,0);
	//alert("edfasfasf"+xx+"asfasfasa")
	var g = new Graph(300,300);
	g.addRow(<%=counters%>);
	g.scale = 1;
	g.setDate();
	g.title = "Vendors";
	...
	g.xLabel="Vendors"
	g.rowText(val);
	g.cursorStyle="style=cursor:hand"
	g.build();
