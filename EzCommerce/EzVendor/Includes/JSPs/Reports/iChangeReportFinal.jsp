<%
	...
	for(int i=0;i<paramCount;i++)
	{
		...
		lowValue[i]=("µ".equals(lowValue[i]))?" ":lowValue[i];
		highValue[i]=("µ".equals(highValue[i]))?" ":highValue[i];


		
		
		if(paramDesc[i].length() == 0 || "null".equals(paramDesc[i]))
		{
		...
		if(IS_DEFAULT)
		{
			...
			{
				if(!(repDefaults.getFieldValueString(0,"DEF_VALUE")== null || "null".equals(repDefaults.getFieldValueString(0,"DEF_VALUE"))))
				{
					from = fromCondition1(repDefaults.getFieldValueString(0,"DEF_VALUE"),1);
				}	
			}	
			else if(repDefaultsCount == 0)
				from = fromCondition1(repDefaults.getFieldValueString(0,"DEF_VALUE"),0);
			else 
				from = fromCondition2(i,repDefaults,lowValue[i]);
		}
		else
		{
			if(IS_DATE)
				from = fromCondition3(i,paramLen[i],lowValue[i]);
			else if(IS_PARAM && "1".equals(paramLen[i]))
			{
				String chkLow =("µ".equals(lowValue[i]) || lowValue[i].trim().length()==0)?"":"checked";
				...
				from = fromCondition4(checkBoxType,checkBoxName,checked,defaultCheckValue,i,chkLow);
			}
			else
			{
				...
				from = fromCondition5(i,mandscript,paramLen[i],lowValue[i]);
			}
		...
		{
			if(IS_DATE)
				to = toCondition1(i,highValue[i]);
			else
				to = toCondition2(i,true,paramLen[i],chkDataType,highValue[i]);
		}
		else
				to = toCondition2(i,false,paramLen[i],chkDataType,highValue[i]);
...
<%!
	...
	private String fromCondition1(String repDefaultValue,int dataValue)
	{
		String FROMCONDITION = "";
		if(dataValue == 1)
			FROMCONDITION = repDefaultValue;
		FROMCONDITION += "<input type='hidden' name='lowValue' value="+repDefaultValue+">";	
		return FROMCONDITION;
	}
	
	
	private String fromCondition2(int i,ReturnObjFromRetrieve repDefaults,String dataBaseValue)
	{
		...
		{
			if(dataBaseValue.equals(repDefaults.getFieldValueString(k,"DEF_VALUE")))
				FROMCONITION += "<option selected value="+repDefaults.getFieldValueString(k,"DEF_VALUE")+">"+repDefaults.getFieldValueString(k,"DEF_VALUE")+"</option>";
			else
	...
	private String fromCondition3(int i,String size,String dataBaseValue)
	{
		String FROMCONDITION = "";
		FROMCONDITION += "<input type='text' name='lowValue' class = 'InputBox' size='"+size+"' maxlength='"+size+"' readonly value='"+dataBaseValue+"' nowrap>";
	...
	private String fromCondition4(String checkBoxType,String checkBoxName,String checked,String defaultCheckValue,int i,String chkValue)
	{
		String FROMCONDITION = "";
		FROMCONDITION += "<input type="+checkBoxType+" name='"+checkBoxName+"' "+checked+" onClick='setLowVal(this,"+i+")' "+chkValue+">";
		FROMCONDITION += "<input type='hidden' name='lowValue' value="+defaultCheckValue+">";
		return FROMCONDITION;
	}
	
	private String fromCondition5(int i,String mandScript,String size,String dataBaseValue)
	{
		String FROMCONDITION = "";
		FROMCONDITION = "<input type='text' name='lowValue' class = 'InputBox' size='"+size+"' maxlength='"+size+"' "+mandScript+" value='"+dataBaseValue+"'>";
		return FROMCONDITION;
	}



	private String toCondition1(int i,String dataBaseValue)
	{
		String TOCONDITION = "";
		TOCONDITION += "<input type='text' name='highValue' class = 'InputBox' size='10' readonly value='"+dataBaseValue+"' nowrap>";
	...
	private String toCondition2(int i,boolean sendExt,String size,String dataType,String dataBaseValue)
	{
		if("null".equals(dataBaseValue))
			dataBaseValue = "";
		String TOCONDITION="";
		if(sendExt == true)
			TOCONDITION += "<input type='text' name='highValue' class = 'InputBox' size='"+size+"' maxlength='"+size+"' onblur='validate(\""+dataType+"\",this,"+i+")' value='"+dataBaseValue+"'>";
		else
