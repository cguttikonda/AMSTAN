...
<%!
	...
	{
		//return "<input type='text' name='lowValue' class = 'InputBox'  style = 'width:100%' maxlength='"+size+"' "+mandScript+" value="+lowDefaultValue+">";
		if(lowDefaultValue == null || "null".equals(lowDefaultValue))
			return "<input type='text' name='lowValue' class = 'InputBox'  style = 'width:100%'>";
		else
			return "<input type=hidden name=lowValue value='"+lowDefaultValue+"'>"+lowDefaultValue;
	}
	...
	private String getFromField(String lowDefaultValue,ReturnObjFromRetrieve reportDefaults,int i)
	{
		...
		for(int k=0;k<reportDefaultCount;k++)
		{
			...
			if(al.contains(defvalue))
				FROMCONITION += "<option selected value="+defvalue+">"+defvalue+"</option>";
			else
				FROMCONITION += "<option value="+defvalue+">"+defvalue+"</option>";
		}	
		FROMCONITION += "</select>";
		FROMCONITION += "<input type='hidden' name='lowValue' value='"+lowDefaultValue+"'>";
	...
	private String getFromField(String lowDefaultValue,String lowValue,int condition,int i,String size,String mandScript)
	{
		...
		if(condition == 1)
		{
			FROM_CONDITION = "<input type='text' name='lowValue' readOnly class='InputBox' size=12   value='"+lowDefaultValue+"' nowrap>";
			FROM_CONDITION += "<a href='javascript:showCal(\"document.addForm.lowValue["+i+"]\",50,580)'><img border=no style='cursor:hand' src='../../Images/Buttons/English/Green/calender.gif' alt='Calender' align='middle'></a>";	
		}	
		else if(condition == 3)
		{
	...
	private String getFromCheckField(String checkboxType,String checkboxName,String checked,String checkBoxValue,int i)
	{
		...
		String FROM_CONDITION = "<input type='"+checkboxType+"' name='"+checkboxName+"' "+checked +" onClick='setLowVal(this,"+i+")'>";
		FROM_CONDITION += "<input type='hidden' name='lowValue' value='µ'>";
		return FROM_CONDITION;
	}
	...
	private String getToField(String highDefaultValue,int condition,int i,String size,char paramDataType)
	{
		...
		if(condition == 1)
		{
			TO_CONDITION  = "<input type='text' name='highValue' readOnly class = 'InputBox'  size=12  value='"+highDefaultValue+"' nowrap>";
			TO_CONDITION  += "<a href='javascript:showCal(\"document.addForm.highValue["+i+"]\",50,580)'><img border=no style='cursor:hand' src='../../Images/Buttons/English/Green/calender.gif' alt='Calender' align='middle'></a>";
		}
		else if(condition == 2)
		{
