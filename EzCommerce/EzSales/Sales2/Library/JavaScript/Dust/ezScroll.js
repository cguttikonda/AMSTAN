...
function scrollInit(adjustWidth)
{
	...
	if (myInitFlag)
	{
		...
		if(positionFlag){
			OuterdivId.style.borderBottom="#006666 3px inset";
...
function myInit()
{
	...
	DynLayerInit()
		...
		scrollWho=InnerBox1
		myScrollHeight =  InnerBox1.scrollHeight
		myScrollHeight=	myScrollHeight-toScrollHeight
		InnerBox1.slideInit()
		scrollWho.onSlideEnd=scrollMessage
...
{
	if( scrollWho.h > (boxBottom-boxTop))
		return true
	else
		return false
}
...
{
		toScrollHeight=boxBottom-boxTop
		myScrollHeight=scrollWho.elm.offsetHeight-toScrollHeight
		if (UpDn=='dn')
		{
...
function pageUpDnMessage()
{
		...
		myScrollHeight= scrollWho.scrollHeight-toScrollHeight
...
if(is.ns)
{
	document.write("#OuterBox1Div {overflow:hidden; position: absolute; visibility: show; z-index: 0;}")
	document.write("#InnerBox1Div {overflow:hidden; position: absolute; visibility: show; z-index: 0;}")
}else{
