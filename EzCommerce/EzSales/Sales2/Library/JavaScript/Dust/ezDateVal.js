...
function VerifyDate( field, form ) {
	...
	rdate = trim(rdate);
      if ( rdate == '' )
	{
		alert(RequiredDatecannotbeempty);
...
function VerifyNum( i ) {
	...
	else
	{
		...
		{
			alert(Quantitymustbegreaterthanzero);
			document.forms[0].elements['ORDER_' + i].select();
			document.forms[0].elements['ORDER_' + i].focus();
			document.returnValue = false;
		}
...
function CheckCart() {
	...
	if (pCount < 2){
		alert(AddLineItemsBeforeCreatingSalesOrder);
		document.returnValue = false;
	}
	else{
		if (ponum == ""){
		alert(EnterPONumberBeforeCreatingSalesOrder);
		document.returnValue = false;
		}
		else{
			document.returnValue = true;
