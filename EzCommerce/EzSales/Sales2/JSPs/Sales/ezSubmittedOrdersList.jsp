...
<Script>	
	function doOnLoad()
	{
		...
		mygrid.setHeader("Sales Order No,Order Date,PO No,Created By,Customer");
		mygrid.setNoHeader(false)
		mygrid.setInitWidthsP("15,20,15,15,35")
		...
		mygrid.init();	
		mygrid.loadXML("ezSubmittedOrdersXML.jsp?orderStatus=<%=orderStatus%>&SearchType=<%=searchType%>&searchPatern=<%=request.getParameter("searchPatern")%>&sortOn=<%=request.getParameter("sortOn")%>&sortOrder=<%=request.getParameter("sortOrder")%>&FDATE="+document.SOForm.FromDate.value+"&TDATE="+document.SOForm.ToDate.value);
	}
	function doOnUnLoad()   
	{
		if (mygrid) 
...
<form name=SOForm method="post" action="../Sales/ezSubmittedOrdersList.jsp">
...
<%
	String display_header = "Submitted "+salordli_L;
	String noDataStatement= "No Submitted Orders to list";
