...
<%
	...
	if(listShipTos_ent!=null && listShipTos_ent.getRowCount()>0)
	{
		...
		rowId = listShipTos_ent.getRowId("EC_ERP_CUST_NO",ShipTo);
	...
	if(cartcount>0)
	{
		...
		for(int i=0;i<cartcount;i++)
		{
			...
			/* Added by MB */
			String erpNetPrice = itemoutTable.getFieldValueString(i,"NetValue");
			System.out.println("MUKESH PRICE "+erpNetPrice);
			log4j.log("MUKESH BABLANI"+itemoutTable.toEzcString(),"W");
			/* end of addition by MB*/
			mfrNr[i]    = mfrNr_A;
			mfrPart[i]  = mfrPart_A;
			eanupc[i]   = eanupc_A;
			matId[i]    = matId_A;
...
<Script>
	...
	function applyPromoCode()
	{
		...
		if(eval(y))
		{
			...
			var url=location.protocol+"//<%=request.getServerName()%>/AST/EzCommerce/EzSales/Sales2/JSPs/Sales/ezAjaxCheckPromoCode.jsp?promoCode="+chkPromoCode;
	...
	function calFreight(obj)
	{
		...
		{
			document.generalForm.shipIdNo.value="";
			...
			else if((funTrim(servType)!="" && funTrim(weight)!="0") || stat=="POSTALCODE")
			{
				...
				var url=location.protocol+"//<%=request.getServerName()%>/AST/EzCommerce/EzSales/Sales2/JSPs/Sales/ezAjaxFreightType.jsp?servType="+servType+"&countryCode="+countryCode+"&zipCode="+zipCode+"&weight="+weight+"&packType="+packType;
		...
		{
			document.generalForm.shipIdNo.value=document.generalForm.shipID.value;
	...
	function selectedShipTos()
	{
		...
		shipCode	= selShipTo.split('#')[6]
		
		document.generalForm.shipToName.value	  = shipAddr
		document.generalForm.streetName.value	  = shipStreet
		document.generalForm.shipToAddress2.value = shipCity
		document.generalForm.shipToState.value	  = shipState	
		document.generalForm.shipToCountry.value  = shipCountry
		document.generalForm.shipToZipcode.value  = shipZip
		document.generalForm.sel_Ship.value  	  = shipCode
...
<input type="hidden" name="sel_Ship" value="">
...
<tr>
<td valign='top'>
<table width="100%" align="left" border="0" border=0 borderColorDark=#ffffff borderColorLight=#006666 cellPadding=2 cellSpacing=1 class="table_list">
  
  <tr class="sectiontableheader"> 
    <th align="left" colspan="2"><h4>Order Information</h4></th>
  </tr>
	<Tr>
    		<td><strong>PO Number</strong></td>
     		<td width="25%" align="left"><input type="text" class=InputBox name="poNo" value=""></td>
     	</tr>
     	<Tr>
     		<td><strong><%=podate_L%></strong></td>
        	<td width="25%"  align="left"><input type="text" class=InputBox name="poDate" id="poDate" value="<%=PODate%>" readonly onClick="blur()" onFocus="blur()" size="12"><%=getDateImage("poDate")%></td>
        </tr>
        <Tr>
    		<td><strong>Req. Del. Date</strong></td>
        	<td width="25%" align="left"><input type="text" class=InputBox name="requiredDate" id="requiredDate" value="<%=requiredDate%>" readonly onClick="blur()" onFocus="blur()" size="12"><%=getDateImageFromToday("requiredDate")%></td>
	</tr>
	<Tr>
		<Td align=left><strong>Freight Type<strong></Td>
		<Td align=left>
		<select name="fServType" id ="ListBoxDiv1" style="width:100%" onChange="calFreight('FTYPE')">
		<option value="">--Select--</option>
		<option value="SP">Third Party Billing</Option>
<%
...
<%
	String shipIdNo = (String)session.getValue("SHIPID");

	if(shipIdNo==null || "null".equals(shipIdNo) || "".equals(shipIdNo)) shipIdNo = "N/A";
%>
	<Tr>
		<Td align=left><strong>Shipping ID<strong></Td>
		<td width="25%" align="left"><input type="text" class=InputBox name="shipIdNo" value="" disabled></td>
	</Tr>
 </table>
 </td> 
 <td valign='top'>
 <table width="100%" align="left" border="0" border=0 borderColorDark=#ffffff borderColorLight=#006666 cellPadding=2 cellSpacing=1 class="table_list">
 	<tr class="sectiontableheader"> 
 	    <th align="left" colspan="8"><h4>Addresses</h4></th>
  	</tr>
	<Tr>
     		<th class="labelcell"  align="left"  colspan = 2>
     		        <%=soldto_L %> Address
			<input type="hidden" name="soldTo" value="<%=SoldTo%>">
			<input type="hidden" name="soldToName" value="<%=agentName%>">
     		</th>
     		<Th class="labelcell"  align="left" colspan = 6  >
     		        <%=shipto_L%> Address
	...
	<Tr>
		...
		<Td>
			<select name="selShipTo" id ="ListBoxDiv1"  onChange="selectedShipTos()" style="width:70%">
			<option value="">------Select------</option>
<%
			for(int l=0;l<listShipTos_ent.getRowCount();l++)
			{
				...
				String shipParamas= ShipToName+"#"+shipAddr1+"#"+shipAddr2+"#"+shipState+"#"+shipCountry+"#"+shipZip+"#"+shipToCode;
%>
				<option value="<%=shipParamas%>"><%=shipToCode%>(<%=ShipToName%>)</option>
<%   
			}
%>
			</Select>
	...
	<Tr>
		...
		<td ><%=agentName%></Td>
                ...
                <Td align=left>Company*:</Td>
                <Td ><input maxlength="64" class='InputBox' type="text" name="shipToName"  </Td>
...
<!--                <Td align=right>Attn.:</Td>
	...
	<Tr>
      		...
      		<Td align=left>Street*:</Td>
      		<Td  colspan=2>
      		<input type="text"  class='InputBox' name="streetName"  maxlength="40" >
      		</Td>
<!--	        <Td align=right>Tel# 1</Td>
	...
	<Tr>
      		...
      		<input type="hidden" name="soldToAddress2" value="<%=billAddr2%>"></Td>
                <Td align=left>City*:</Td>
                <Td colspan=2>
                	<input type="text"  class='InputBox' name="shipToAddress2"  maxlength="40" >
                </Td>
...
%>
	...
	<Tr>
		...
		<Td align=left>Postal Code*:</Td>
		<Td >
			<input type="text" class='InputBox' size = '10' name="shipToZipcode"  maxlength="10" >
		</Td>
	</Tr>
<!--	<Tr>
	...
	<input type="hidden" name="shipID" value="<%=shipIdNo%>">
	<input type="hidden" name="specialShIns" value="">
...
<Tr>
	...
	<Th width="11%" valign="top"><%=delSchedue_L%></Th>
...
<%
	...
	if(cartcount>0)
	{
		...
		log4j.log("itemCatalogs::::::::"+itemCatalogs,"W");
				...
				while(itemSt.hasMoreTokens())
				{
					try
					{
						...
						//if(custMatFlg){
							...
							// Mukesh Bablani subTot = ((new java.math.BigDecimal(prodQty)).multiply(new java.math.BigDecimal(itemListPrice))).setScale(2,java.math.BigDecimal.ROUND_HALF_UP).toString();
			...
			log4j.log("prodCodeprodCode>>>>"+prodCode,"W");
			log4j.log("custprodCodecustprodCode>>>>"+custprodCode,"W");
			log4j.log("itemVenCatalogitemVenCatalog>>>>"+itemVenCatalog,"W");
			log4j.log("itemCatalogsitemCatalogs>>>>"+itemCatalogs,"W");
			
			prodCode=custprodCode;
		...
		if(catDefaultsHt.get(itemVenCatalog)!=null) catDefaultsStr = (String)catDefaultsHt.get(itemVenCatalog);
...
%>
		<Tr>
			...
			<Td width="22%" align="left" title="<%=a%>" <%=tdColor%>>
				&nbsp;<input type="text" name="prodDesc" size="42" <%=colTone%>  readonly value="<%=prodDesc%>">
...
%>
				...
				<input type="hidden" name="itemSAPPrice" value="<%=priceCurr.trim()%>">
...
%>			
			...
			<Td width="10%" align="right" <%=tdColor%>>
				<input type="text" class="InputBox" size="8" name="itemListPrice" value="<%=priceCurr.trim()%>" style="text-align:right" onBlur="verifyField(this,'Please enter valid price');">
			</Td>
...
<Div id="showTot" style="visibility:visible">
	...
	<Tr>
		...
		<Tr>
			...
			<Td width="42%" align=right>&nbsp;<input type="text" name="freightInsHid" <%=colTone%> size="10" readonly value="TBD" style="text-align:right">
			<input type="hidden" name="freightIns" <%=colTone%> size="10" readonly value="0.00" style="text-align:right">
			</Td>
		</Tr>
		<Tr>
		...
		if("CU".equals(UserRole.toUpperCase()))
		{
			if("Y".equals(isSubUser))
			{
				...
				if("VEDIT".equals(suAuth))
				{
					...
					buttonMethod.add("formSubmit(\"ezAddSaveSales.jsp\",\"NEGOTIATED\")");
				}
			}
			else
			{
