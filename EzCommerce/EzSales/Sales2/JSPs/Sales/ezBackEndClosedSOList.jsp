...
%>	
			...
			if(custVal=="")
			{
				mygrid.loadXML("ezClosedOrdersXML.jsp?RefDocType=P&FromDate="+fromDate+"&ToDate="+toDate+"&ezMonths="+myVal);			
			}
			else
			{
				mygrid.loadXML("ezClosedOrdersXML.jsp?RefDocType=P&FromDate="+fromDate+"&ToDate="+toDate+"&customer="+custVal+"&ezMonths="+myVal+"&custStr="+custStr);			
			}
			<%}else{%>
			if(custVal=="")
			{
				mygrid.loadXML("ezClosedOrdersXML.jsp?RefDocType=P&FromDate="+fromDate+"&ToDate="+toDate);	
			}
			else
			{
				mygrid.loadXML("ezClosedOrdersXML.jsp?RefDocType=P&FromDate="+fromDate+"&ToDate="+toDate+"&customer="+custVal+"&custStr="+custStr);	
			}
...
<%
	String display_header = "Closed Sales Order List";
	String noDataStatement= "No Closed Orders to list";
...
%>
		...
		<tr>
			<td class=displayalert align ="center" colspan ="4">
<%
			if(!"CU".equals(UserRole.trim()))
			{
%>			
				Please select Customer(s), dates and click on 'GO' Button.
<%
			}
			else
			{
%>			
				Please select dates and click on 'GO' Button.
<%
			}
%>			
			</td>
