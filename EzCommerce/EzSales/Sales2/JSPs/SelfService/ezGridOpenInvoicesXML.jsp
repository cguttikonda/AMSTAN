...
<%	
	...
	//ezc.ezcommon.EzLog4j.log("orgItems>>>>>>>>>>>>"+orgItems.toEzcString() ,"I");
	...
	String custbillNum="",invNum="",delDocNo="",invAm="",pd="",bd="",delDocStr="",custPoNo="";
	ezc.ezbasicutil.EzCurrencyFormat myFormat = new ezc.ezbasicutil.EzCurrencyFormat();
	String  formatkey=(String)session.getValue("formatKey");	

	//out.println("<?xml version=\"1.0\"?>");
	//out.println("<rows>");
	String  tcustbillNum = "",tdelDocNo= "";
	if(invcount>0)
	{
		for(int i=0;i<invcount;i++) 
		{
			...
			custPoNo	= orgItems.getFieldValueString(i,"CUSTOMERPONO");
			...
			try
			{
					tdelDocNo=Long.parseLong(delDocNo)+"";
			}
			catch(Exception e1)
			{
				tdelDocNo	=	 delDocNo;
			...
			if(!"".equals(delDocNo))
				delDocStr = "<cell><![CDATA[<nobr><a href='../DeliverySchedules/ezViewDeliveryDetails.jsp?DeliveryNo="+delDocNo+"'>"+tdelDocNo+"</a></nobr>]]></cell>";
			else	
				delDocStr = "<cell>"+delDocNo+"</cell>";

			out.println("<row id='"+custbillNum+"'><cell><![CDATA[<nobr><a href='ezInvoiceDetails.jsp?custInvNo="+custbillNum+"&amp;sapInvNo="+invNum+"&amp;InvDate="+pd+"'>"+tcustbillNum+"</a></nobr>]]></cell>"+delDocStr+"<cell>"+custPoNo+"</cell><cell>"+pd+"</cell><cell>"+bd+"</cell><cell>"+myFormat.getCurrencyString(invAm)+"</cell></row>");
		}
