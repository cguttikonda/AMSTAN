...
<%
	...
	String catalogStr    = "10390$$A Y McDonald$$V";
	...
	int rSetSize=0;
		...
		if(retObj!=null && retObj.getRowCount()>0){
			String rFields[] = new String[]{"EMM_NO","EMD_DESC","EMM_CATALOG_NO","EMM_MANUFACTURER","EMM_UNIT_PRICE","EMM_UNIT_OF_MEASURE","EMM_IMAGE_PATH"};
			rSet = new ResultSet();
 ...
 <head>
   ...
   <Script>
	...
	function SendQuery()
	{
		...
		var url="";
		url=location.protocol+"//<%=request.getServerName()%>/AFS/EzCommerce/EzSales/Sales2/JSPs/ShoppingCart/ezAjaxAddCartNew.jsp?product="+product.value+"&quantity="+quantity.value+"&catalog="+catalog.value+"&cartQty="+cartQty.value;
...
{ 
	  ...
	  var pgSize      = document.myForm.pageSize.value;
	  
	  var pageSizeVal = document.myForm.pageSize.value; 

	  if(pageSizeVal>0)
	  {
	  	//document.myForm.action="disPlayTest.jsp?catalogStr="+catalogStr+"&searchStr="+searchStr+"&searchType="+searchType+"&vendCatalog="+vendCatalog+"&itemNo="+itemNo+"&itemDesc="+itemDesc+"&gProdStr="+gProdStr;
	  	//document.myForm.submit(); 
	  	document.location.href="disPlayTest.jsp?catalogStr="+catalogStr+"&searchStr="+searchStr+"&searchType="+searchType+"&vendCatalog="+vendCatalog+"&itemNo="+itemNo+"&itemDesc="+itemDesc+"&gProdStr="+gProdStr+"&pageSize="+pgSize;
	  } 
	  else
	  {
...
function popUp(prdCode,vendCatalog)
{
	 var qryString = "ezProdDetails.jsp?prdCode="+prdCode+"&vendCatalog="+vendCatalog;  
...
function showATPDetails(prodCode,prdDesc)
{
	myurl ="../Sales/ezGetATPDetails.jsp?ProductCode="+prodCode+"&prdDesc="+prdDesc
	...
	if(rSet!=null){
		...
		if(cartRows>0)
		{
		   for(int i=0;i<cartRows;i++){ 
		   	cartMat.put(Cart.getMaterialNumber(i),Cart.getOrderQty(i)); 
		   }
...
<%
			...
			if(cartMat!=null){
				cartQty =(String)cartMat.get(prdCode);
...
%>
			...
			<b> <a href="JavaScript:popUp('<%=prdCode%>','<%=rsData.getColumn3()%>')"> <%=prodT%></a></b>   
			<%=rsData.getColumn2()%> <br>$<%=rsData.getColumn5()%> (List Price) 
			<br>
			<a href="JavaScript:uploadDocs('<%=prdCode%>','<%=rsData.getColumn3()%>')">Upload Docs</a>
			...
			<display:column title="Brand"        		     style="width:35%;height:50;text-align:left">
			<%=(rsData.getColumn4()).toString()%>  
			
			<input type ="hidden" name="catalog_<%=rowNum%>" value="<%=rsData.getColumn3()%>">
			</display:column>          
			
			<display:column title="Image"       		     style="width:25%;height:50;text-align:center" >
<%                      
...
%>
			...
			<display:column title="ATP"    style="width:20%;height:50;text-align:center" > 
<%			
...
%>
 			<a href="JavaScript:showATPDetails('<%=prdCode%>','<%=rsData.getColumn2()%>')"> ATP</a>

<%
                        }
%>
			</display:column> 
			<display:column title="Add to Cart" style="width:10%;height:50;text-align:left;cursor:hand"><image src="/AFS/img/addCart.JPG" title="Click here to Add To Cart" onClick="addToCart('<%=rowNum%>')"/>
 ...
 <%
    if(prodDesc1!=null && ("".equals(searchType) || "GCP".equals(searchType)))
    {
 %>
