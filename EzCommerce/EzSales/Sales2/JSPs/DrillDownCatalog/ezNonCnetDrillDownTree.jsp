...
<%@ include file="../../../Includes/JSPs/DrillDownCatalog/iNonCnetDrillDownTree.jsp" %> 
<html>
<head>
...
{	alertall(idx)	
	parent.document.getElementById("subDisplay").src="../NonCnet/ezNonCnetPrdListByCategoryWait.jsp?categoryID="+catid+"&categoryDesc="+desc+"&STYPE=BY_CAT";
	
}
function selectFav(cStr,idx)
{
	alertall(idx)	
	parent.document.getElementById("subDisplay").src="ezNonCnetViewPCDetails.jsp?catalogStr="+cStr; 
}
function gSearch(idx)
{
	alertall(idx)	
	parent.document.getElementById("subDisplay").src="../NonCnet/ezNonCnetSearch.jsp";
}
...
<%
	if(retCatCount>0)
	{
		...
		{
		ctr++;
		String tempDesc = retcat.getFieldValueString(k,"EMM_FAMILY");
		tempDesc = tempDesc.replaceAll("&","@");
		
%>
		<li><a href="javascript:selectCat('<%=retcat.getFieldValueString(k,"EMM_TYPE")%>','<%=tempDesc%>','<%=ctr%>')" ><span id="mspan<%=ctr%>"><%=retcat.getFieldValueString(k,"EMM_FAMILY")%></span></a></li>
<%
		}
	}
	else
