...
<%
	...
	String[] itemvenCatalog	= request.getParameterValues("venCatalogNo");
	...
	log4j.log("========>product_1=====>"+product_1.length,"D");
	log4j.log("========>commitedQty_1=====>"+commitedQty_1.length,"D");   
	
	String msgExt ="";
	String msgInt = "";
	String subject= "";  
	
	String address1_freq = request.getParameter("shipToAddress1");  
	...
	log4j.log("address1_freq>>>>>>>>>>>>>>"+address1_freq,"W");
...
<%      }
	...
	log4j.log("SAPnumberSAPnumberSAPnumberSAPnumber::"+SAPnumber,"W");     
		...
		if(address1_freq!=null){
			ezShipToStructure.setStreet(address1_freq.replace('¥',','));
		...
		try
		{
			...
			log4j.log("poDatpoDatpoDatpoDat::"+e,"W");
	       	...
	       	String line,product,productDesc,name,desiredQty,commitedQty,desiredDate,commitedDate,desiredPrice,commitedPrice,value,plant,Currency,custMat,itemVendCatalog;
       		String strFOC ="";
       		int IntFOC=0;
       		int newRows =1;
       		...
       		for(int i=0;i<Rows;i++)
       		{
	      		...
	      		line 		= lineNo_1[i];
	       		...
	       		itemVendCatalog = itemvenCatalog[i];
	       		
	       		log4j.log("productDescproductDesc**********::"+productDesc,"W");
	       		log4j.log("custMatcustMatcustMatcustMat**********::"+custMat,"W");
			...
			desDate  = new java.util.GregorianCalendar(yearReq,monthReq-1,dateReq);  
	       		...
	       		itemRow.setIncoterms2(EanUPC[i]);  //EanUPC
			itemRow.setInvoice(mfrNr[i]);     //MfrNR   	       		
			...
			else
			{
       		...
       		log4j.log("itemInTable Count>>>>>>..::"+retSchChangesLocal.toEzcString(),"W");
		...
		ezcParams.setObject(eziWfDocHis);
			...
			try{
				try
				{
					sDocNumber = Integer.parseInt(sDocNumber)+"";
					soNo = soNo.trim();
				}
				catch(Exception ee)
				{}
				...
				{
					subject= "New Web Sales Order "+soNo+"("+sDocNumber+") has been created.";				
					msgExt = "New Web Sales Order "+soNo+"("+sDocNumber+") has been created.<BR> PO NO &nbsp;&nbsp;: "+ ezHeader.getPoNr()+".<BR>Customer  : " + ezSoldToStructure.getName()  + "(" + ezSoldToStructure.getSoldTo() + ")<BR><BR> Regards,<BR> &nbsp;"+Session.getUserId();
...
<%				}
        ...
        {
               	status="SAVED";       		
       		msg   = "Web Order No:<font color=red>"+soNo+"</font> has been Saved.<br>To Submit it to SAP please go to Saved orders in the Orders Menu";       		
       	}
       	...
       	else if("TRANSFERED".equalsIgnoreCase(status))
       	{
			...
			}catch(Exception e)
			{
       			...
       			{       				 
       				msg = "Web Order No: <font color=red>"+soNo+"</font> has been Submitted to SAP with Sales Order no(s): <font color=red>"+sDocNumber+"</font>";
