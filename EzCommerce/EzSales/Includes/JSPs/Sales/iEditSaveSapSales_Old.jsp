...
<%
	...
	//String notesHId[] = new String[]{"0013","0014","0015","0010","0011","0001","0003","Z005"};
         ...
         for (int j=0; j < Rows; j++) 
          { 
			...
			iteminTablexChangeSim.appendRow(iteminTablexRowhangeSim);
					...
					/*double bConfirmedPrice =0;
					bConfirmedPrice = new Double( lineDesPrice[k] ).doubleValue();
					java.math.BigDecimal  ConfirmedPriceB = new java.math.BigDecimal( bConfirmedPrice);
					*/
		     ...
		     /* String lineNotes = request.getParameter("lineNotes_"+k);
			EzStringTokenizer st = new EzStringTokenizer(lineNotes,"@@");
			int lineCnt = st.getTokens().size();

			String x=null;		
			String notes[] = new String[lineCnt];
			String notesId[] = new String[]{"0004","0006","0001","0002","0003","0007"};
		        */

			/*for(int z=0;z<lineCnt;z++)
			{
				try{
		 			notes[z] =(String) st.getTokens().elementAt(z);
					if(!"None".equals(notes[z]))
					{
						EzStringTokenizer notSt = new EzStringTokenizer(notes[z],"/n");
						int ct =notSt.getTokens().size();

						if(ct == 0)
						{
							String chkLin = notes[z];
							if(chkLin.length() >130)
							{
								int len =(int)Math.ceil(chkLin.length()/130);
								for(int l=0;l<len;l++)
								{
									...
									ezctextRowSim = new EzBapistextTableRow();
									ezctextRowSim.setDocNumber(lineDocs[k]);
									ezctextRowSim.setItmNumber(line);
									ezctextRowSim.setTextId(notesId[z]);
									ezctextRowSim.setLanguage("E");
									ezctextRowSim.setTextLine(cutLen);
									ezctextRowSim.setColFormat("*");
									ezctextRowSim.setFunction("002");
						...
						{
							for(int lin=0;lin<ct;lin++)
							{
								String chkLin = (String)notSt.getTokens().elementAt(lin);
								if(chkLin.length() >130)
								{
									int len =(int)Math.ceil(chkLin.length()/130);
									for(int l=0;l<len;l++)
...
//*********************	              if (!OrderQuantity.equals(""))
		   			{ 
							...
							condTableRowChange.setCondType("PR00");
					...
					for(int z=0;z<lineCnt;z++)
					{
						try{
							...
							if(!"None".equals(notes[z]))
							{
								EzStringTokenizer notSt = new EzStringTokenizer(notes[z],"\n");
								int ct =notSt.getTokens().size();
								if(ct == 0)
								{
									ezctextRow = new EzBapistextTableRow();
									ezctextRow.setDocNumber(backEndNo);
									ezctextRow.setItmNumber(line);
									ezctextRow.setTextId(notesId[z]);
									ezctextRow.setLanguage("E");
									ezctextRow.setTextLine(notes[z]);
									ezctextRow.setColFormat("*");
									ezctextRow.setFunction("002");
									ezctextTable.appendRow(ezctextRow);
								}else
								{
									for(int lin=0;lin<ct;lin++)
									{
										...
										ezctextRow.setTextId(notesId[z]);
										ezctextRow.setLanguage("E");
										ezctextRow.setTextLine((String)notSt.getTokens().elementAt(lin));
										ezctextRow.setColFormat("*");
										ezctextRow.setFunction("002");
										ezctextTable.appendRow(ezctextRow);
									}
