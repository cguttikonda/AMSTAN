...
//*************************Start of Declarations*******************************************/
	...
	String salesGroup = (String)session.getValue("CU_SALESGROUP");
 	...
 	java.util.ArrayList uProdCat = new java.util.ArrayList();

 	for(int i=0;i<prodCodeLength;i++)
 	{
		if(!uProdCat.contains(itemVenCatalog[i]))
		{
			uProdCat.add(itemVenCatalog[i]);
		}
	}

ArrayList multiOrders_A = new ArrayList();

for(int us=0;us<uProdCat.size();us++)
{
	sDocNumber = "";
	...
	if(PartnNum!=null)
	{
		...
		if(listShipTos!=null && listShipTos.getRowCount()>0)
		{
			...
			if(streetName!=null)
			{
				...
				aRow2.setName1(shipToName_freq);
				aRow2.setCountrykey(setSalVal.getShipToCountry());
			}
			else
			{
...
//****************************Setting Items conditions and delivery schedules*********************************************
	...
	{
		if(itemVenCatalog[j].equals((String)uProdCat.get(us)))
		{
...
//******************************setting  items*********************************************
			...
			aItemRowConfirm.setShipTo(setSalVal.getShipTo());
			...
			aItemRowsim.setShipTo(setSalVal.getShipTo());
