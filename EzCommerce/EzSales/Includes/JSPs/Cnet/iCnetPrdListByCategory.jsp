...
<%
	...
	{
		cnetParams.setStatus("GET_PRDS_COUNT_BY_CATEGORY");
		...
		retCatCntObj = (ReturnObjFromRetrieve)CnetManager.ezGetCnetProductsByStatus(ezcpparams);
		...
		cnetParams.setStatus("GET_PRDS_BY_CATEGORY");
		cnetParams.setQuery(mfrStr);
		isValid = true;
		cnetParams.setCategoryID(categoryID);
	}
	if("BY_CAT_PRDORDESC".equals(STYPE))
	{
		if(prodDesc1!=null && !"".equals(prodDesc1.trim()))
		{
			...
			/*else
			{
				cnetParams.setStatus("GET_PRDS_COUNT_BY_SEARCH");
				cnetParams.setCategoryID(categoryID);
				cnetParams.setQuery(mfrStr+"and (cds_Prod.MfPN like '%"+tempDesc+"%' or cds_Stdnez.Description like '%"+tempDesc+"%')");
				ezcpparams.setObject(cnetParams);
				ezcpparams.setLocalStore("Y");
	...
	else
	{
		...
		if(categoryID!=null)
		{
			...
			if(retMfr!=null)
			{
				...
				{
					String mfrNum = retMfr.getFieldValueString(i,"MfID");
					String mfrDescName = retMfr.getFieldValueString(i,"Text");
