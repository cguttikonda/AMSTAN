...
function dhtmlXGridObject(id){
 ...
 this.hdr.style.border="1px solid #FFAFAF";
 ...
 this.hdrBox.style.visibility = "hidden";
 ...
 this.setColSpan = function(rows,cols)
 {
 	this.colSpanRows = rows;
 	this.colSpanCols = cols;
 }
 
 this.setColumnSpan = function()
 {
	 /*
	 if(this.colSpanRows.length > 0)
	 {
		var tb = this.obj.firstChild;
		var rowsChk;
		if(this.colSpanRows.indexOf(",") != -1)
		{
			rowsChk = this.colSpanRows.split("#")
			for(i=0;i<rowsChk.length;i++)
			{
				for(var j=0;j<this.rowsCol.length;j++)
				{
					if(tb.childNodes[j].childNodes[0].innerHTML == rowsChk[i])
					{
						alert(tb.childNodes[j].childNodes[0].innerHTML)
						tb.childNodes[j].childNodes[0].colSpan = parseInt(this.colSpanCols)
						break;
					}	
				}			
			}
...
this.init = function(fl)
{
 ...
 this.sortImg.style.left = this.sortImg.defLeft;
}
}
...
if(rId == 0)
{
	...
	if(document.getElementById("BackButtonDiv") != null)
		document.getElementById("BackButtonDiv").style.visibility ='visible';
}
else
{
	this.hdrBox.style.visibility ='visible';
	document.getElementById("gridbox").style.visibility ='visible';	
	if(document.getElementById("ButtonDiv") != null)
		document.getElementById("ButtonDiv").style.visibility ='visible';
	if(document.getElementById("BackButtonDiv") != null)
		document.getElementById("BackButtonDiv").style.visibility ='hidden';
}	
document.getElementById("dataRetrieve").style.visibility ='hidden';
//document.getElementById("gridbox").style.visibility ='visible';	

if(setAlternateColors)
	this.setAlternateColors(setAlternateColors)
document.getElementById("gridbox").style.width=this.objBuf.style.width

this.setColumnSpan();


/*var table = this.objBox
var rows = table.getElementsByTagName("tr");
var rLen = rows.length;
for(i=0;i<rLen;i++)
{
	var rowIdStr = rowsCol[i].getAttribute("id");
	if(rId.indexOf("#-") >= 0)
	{
		rows[i].firstChild.innerHTML = "<img src='"+this.imgURL+"item_chk0_dis.gif'>";
	}	
}*/
...
this.sortRows = function(col,type,order)
{
	...
	var totalRows = this.rowsCol.length;
	var tempRowIndex;
	var totalRow = 0
	for(var i=0;i<this.rowsCol.length;i++)
	{
		if(tb.childNodes[i].childNodes[0].innerHTML == 'Total')
		{
			tempRowIndex = i;
			break;
		}	
	...
	var diffRows = totalRows - tempRowIndex;
	for(i=0;i<diffRows;i++)
	{
		if(tb.childNodes[i].childNodes[0].innerHTML == 'Total')
			this.moveRowDown(totalRows-1);
	}
	...
	for(var i=0;i<this.rowsCol.length;i++)
	{
		if(tb.childNodes[i].childNodes[0].innerHTML == 'Taxes,Duties,Levies')
		{
			tempRowIndex = i;
			break;
		}	
	...
	tempRowIndex = tempRowIndex+1
	diffRows = totalRows - tempRowIndex;
	for(i=0;i<diffRows;i++)
	{
		if(tb.childNodes[i].childNodes[0].innerHTML == 'Taxes,Duties,Levies')
			this.moveRowDown(totalRows-2);
	}
...
{
	alert('Caught error in MoveRowUp method: '+(er.description||er))
}
}
...
{
	alert('Caught error in MoveRowDown method: '+er.description)
}
}
 ...
 var c = this.getRowById(row_id).childNodes[col];
 return eval("new eXcell_"+this.cellType[col]+"(c)");
}
}
...
this.sortField = function(ind,repeatFl)
{
	...
	if(el.innerHTML != "&nbsp;" && el.innerHTML.indexOf("<IMG")==-1 && el.innerHTML.indexOf("checkAll") == -1)
	{
 ...
 if(!_isOpera)
{
 ...
 //this.obj.onmousemove = new Function("e","try{if(!this.grid.editor){var c = this.grid.getFirstParentOfType(e?e.target:event.srcElement,'TD');var r = c.parentNode;var ced = this.grid.cells(r.idd,c._cellIndex);(e?e.target:event.srcElement).title = ced.getTitle?ced.getTitle():ced.getValue()}}catch(er){}");
 this.obj.onmousemove = new Function("e","try{if(!this.grid.editor){var c = this.grid.getFirstParentOfType(e?e.target:event.srcElement,'TD');var r = c.parentNode;var ced = this.grid.cells(r.idd,c._cellIndex);}}catch(er){}");
 ...
 document.onkeydown = new Function("e","document.getElementById('"+this.entBox.id+"').grid.doKey(e||window.event)");
 ...
 /*** Satya ***/
 this.setAlternateColors = function(f1)
 {          
 	if(f1)
 	{
 		var table = this.objBox
