...
function _writeLegendGraph() 
{
	alert("hello")
	var st = "";
	st += "<TABLE BORDER=3 CELLSPACING=0 CELLPADDING=4 bgcolor=Yellow><TR><TD><FONT FACE='Arial,Helvetica' SIZE=-1>";
...
function _buildRegGraph(g, doc) 
{
	...
	if(g.posMax > 0) 
	{
		...
		//if(g.bgColor) str += " BGCOLOR=\""+#FFFFFF+"\"";
		//adding usha
		//if(g.bgColor) str += " BGCOLOR='#336699'";
		//str += " BGCOLOR='#D7D7D7'"
		str += ">";
		...
		for(var j = 0; j < g.rows[0].length; j++) 
		{
			for(var i = 0; i < g.rows.length; i++) 
			{
				if(parseInt(g.vscale*g.rows[i][j]) > 0) 
				{
					...
					str += "<table><tr><td>"+g.rows[i][j]+"</td></tr><tr><td onClick=fun1('"+g.dates[j]+"')><IMG BORDER=0 SRC=\""+i+".gif\" ";
					str += "ALT=\"";
...
var str = "";
str += "<TABLE BORDER=2 CELLPADDING=0 CELLSPACING=0>\n";
if(g.title) {
str += "<TR>\n";
if(g.scale) str += "<TD COLSPAN=3></TD>\n";
if(g.yLabel) str += "<TD></TD>\n";
