function ezChkOperator()
{
	var obj1=document.addForm.lowValue
	var obj2=document.addForm.highValue
	var obj3=document.addForm.paramOpt
	
	for(var r=0;r<obj1.length;r++)
	{
		if(obj1[r].type == 'text' && obj2[r].type == 'text')
		{
		
			if(funTrim(obj1[r].value) != "" && funTrim(obj2[r].value) != "")
			{
			
				for(s=0;s<obj3[r].length;s++)
				{
					if(obj3[r].options[s].value == 'BT')
					{
						obj3[r].selectedIndex = s
						break;
					}
				}	
			}
...
function ezBack()
{
	history.go(-1);
      /*ezForm=document.addForm
      ezForm.action="ezListReportCU.jsp?system=<%=system%>&sysDesc=<%=sysDesc%>&reportDomain=<%=reportDomain%>"
      ezForm.submit()*/
}
function setLowVal(obj,ind)
{
	...
	if(obj.type=="radio")
	{
		...
		myLen=eval("document.addForm."+obj.name + ".length")
		
		for(i=0;i<myLen;i++)
		{
			
			myObj1=eval("document.addForm."+obj.name + "[" + i + "]")
			
			if(myObj1.checked)
			{
				
				document.addForm.lowValue[myObj1.ezLowIndex].value='X';
			}
			else
			{
			 
				document.addForm.lowValue[myObj1.ezLowIndex].value='µ';
			}
		}
	}	    
...
function showConfirmDiv()
{
	...
	addrowsText += '<img src="../../Images/Buttons/<%= ButtonDir%>/ok.gif" border="none" style="cursor:pointer;cursor:hand" onClick="HideNetDiv()">'
