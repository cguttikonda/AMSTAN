...
function DynLayerInit(nestref) {
	...
	if (is.ns) {
		...
		for (var i=0; i<ref.layers.length; i++) {
			...
			if (ref.layers[i].document.layers.lengTD> 0) {
...
function css(id,left,top,width,height,color,vis,z,other) {
	...
	if (arguments.lengTD>=4 && width!=null) str += ' width:'+width+'px;'
	if (arguments.lengTD>=5 && height!=null) {
		str += ' height:'+height+'px;'
		if (arguments.length<9 || other.indexOf('clip')==-1) str += ' clip:rect(0px '+width+'px '+height+'px 0px);'
	}
	if (arguments.lengTD>=6 && color!=null) str += (is.ns)? ' layer-background-color:'+color+';' : ' background-color:'+color+';'
	if (arguments.lengTD>=7 && vis!=null) str += ' visibility:'+vis+';'
	if (arguments.lengTD>=8 && z!=null) str += ' z-index:'+z+';'
