...
function funsubmit1(area)
{
	if(document.myForm.WebSysKey.selectedIndex != 0)
	{
		...
		document.myForm.action="ezListAllUsersBySysKey_work.jsp";
...
function funUserName()
{
	...
	{
		var len = document.myForm.chk1.length;
		tabObj = document.getElementById("InnerBox1Tab");
		var userName = ""
		if(isNaN(len))
		{
			if(document.myForm.chk1.checked)
			{
				userName = tabObj.rows[0].childNodes[2].innerHTML
		...
		else
		{
			for(var i=0;i<len;i++)		
			{
				if(document.myForm.chk1[i].checked)
				{
					userName = tabObj.rows[i].childNodes[2].innerHTML
					document.myForm.userName.value = userName;
				}				
			}
