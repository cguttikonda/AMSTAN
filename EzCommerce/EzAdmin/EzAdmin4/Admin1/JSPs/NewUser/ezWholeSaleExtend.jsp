...
<%
		...
		String SysKey="",BusPartner="",pSoldTo="",genNewEzc = "";
        	...
        	Vector alSyskey = new Vector();
		Vector alCust = new Vector();
		HashMap custHM = new HashMap();

		for(int i=0;i<syskey.length;i++)
		{
			alSyskey.add(syskey[i]);
		}

		for(int c=0;c<customer.length;c++)
		{
			String custCode  =  customer[c].split("¥")[0];		
			String sysCode   =  customer[c].split("¥")[1];

			if(alSyskey.contains(sysCode))
				alCust.add(customer[c]);			
		}

		//out.println("alCust:::::"+alCust);

		/*for(int al=0;al<alCust.size();al++)
		{

			String custCode  =  ((String)alCust.get(al)).split("¥")[0];		
			String sysCode   =  ((String)alCust.get(al)).split("¥")[1];

			//out.println("custCode:::"+custCode);
			//out.println("sysCode:::"+sysCode);

			if(!custHM.containsKey(custCode))
			{

				custHM.put(custCode,sysCode);        
			}
			else{
				String sysKey = (String)custHM.get(custCode);

				sysKey +="#####"+sysCode;

				custHM.put(custCode,sysKey);
			}		
		}
		
		Set setCustHM = custHM.entrySet(); 
				Iterator iCustHM = setCustHM.iterator(); 
				while(iCustHM.hasNext()) 
				{ 
					Map.Entry mCustHM = (Map.Entry)iCustHM.next(); 
					out.print(mCustHM.getKey() + " : "); 
					out.println(mCustHM.getValue()); 
		} */
			...
			String exlCust = customer[0].split("¥")[0];
		
			mySynch.ERPSOLDTO=customer[0].split("¥")[0];
			...
			mySynch.ezAreas=mySyskey;
		   	...
		   	String bpnumber=mySynch.addBP();  // adding Business Partner
		   	
	   		//out.println("bpnumberbpnumber::"+bpnumber);
	   		
	   		if(bpnumber!=null && !"null".equals(bpnumber) && !"".equals(bpnumber))
			{
				//mySynch.addBPSysAuth(bpnumber,bpSysAuth);
				mySynch.addBPSysInAuth(bpnumber,bpIndAuth);
											
				miscParams.setIdenKey("MISC_UPDATE");
				String query="UPDATE EZC_CUSTOMER_ADDR SET ECA_TELEBOX_NO = '"+hierCode+"',ECA_PARTNER_LISTBOX_DESC = '"+partnerName+"',ECA_NAME = '"+partnerName+"',ECA_COMPANY_NAME='"+partnerName+"' WHERE ECA_NO ='"+bpnumber+"'";
	...
	pSoldTo  =  ((String)alCust.get(al)).split("¥")[0];		
	SysKey   =  ((String)alCust.get(al)).split("¥")[1];
	
	//out.println("pSoldTo:::"+pSoldTo);
	//out.println("SysKey:::"+SysKey);
	
	
	
	if(!exlCust.equals(pSoldTo))
	{
	
		miscParams.setIdenKey("MISC_INSERT");
		String query="INSERT INTO EZC_USER_DEFAULTS VALUES('"+userId.toUpperCase()+"','"+SysKey+"',NULL,'SOLDTOPARTY','"+pSoldTo+"','N',NULL)";
	...
	if(PartNum!=null)
	{
		...
		if(sysKey!=null && soldTo!=null)
		{
			...
			EzcParams mainParams3 = new EzcParams(false);
			mainParams3.setObject(adminUtilsParams);
			Session.prepareParams(mainParams3);
	
			partnersRet = (ReturnObjFromRetrieve)AUM.getUsersByPartnerValueAndArea(mainParams3);
		}	
