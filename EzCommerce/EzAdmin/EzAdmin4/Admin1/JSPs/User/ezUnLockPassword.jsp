...
<%
	...
	//Connection connection = DriverManager.getConnection("jdbc:microsoft:sqlserver://localhost:1433;DatabaseName=ezranbaxypartners;SelectMethod=direct","ezranbaxypartner","test");
	java.util.ResourceBundle siteProps_A = java.util.ResourceBundle.getBundle("Site");
	
	String SiteNo_A = (String)session.getValue("ConnGroup");
	
	if(SiteNo_A==null || "null".equals(SiteNo_A) || "".equals(SiteNo_A))
		SiteNo_A = siteProps_A.getString("DBGRP");
	
	
	
	Class.forName(siteProps_A.getString("Driver_"+SiteNo_A));
	Connection connection = DriverManager.getConnection(siteProps_A.getString("ConnectString_"+SiteNo_A),siteProps_A.getString("UserId_"+SiteNo_A),siteProps_A.getString("Password_"+SiteNo_A));
	...
	if(userExist)
	{
		...
		if(allowUnLk)
		{
			...
			if("U".equals(PwdFlg.trim()) && alowUl)
			{
				...
				executeStmt = true;
				statusMsg = "User Id : "+userId+"  is Unlocked successfully.";
			}
			else
			{
				statusMsg = "User Id : "+userId+" has not been locked.";
			}
		}
		else
		if("U".equals(PwdFlg.trim()))
		{
			statusMsg = "User Id : "+userId+" is already Unlocked.";
		}
		...
		{	
			if("L".equals(PwdFlg.trim()))
			{
				...
				executeStmt = true;				
				statusMsg = "User Id : "+userId+"  is Locked successfully.";
			}
		}
		else
		if("L".equals(PwdFlg.trim()))
		{
			statusMsg = "User Id : "+userId+" is already Locked.";
		}
