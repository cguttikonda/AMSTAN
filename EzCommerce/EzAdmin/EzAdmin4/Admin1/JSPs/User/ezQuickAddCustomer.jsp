...
<%
	...
	bpIndAuth.put("SALES_SYS_IND","Sales System Independant Role");
	...
	if(bpnumber!=null && !"null".equals(bpnumber))
	{
		...
		for(int i=0;i<syskey.length;i++)
		{
			...
			if(! mySynch.getCustomersFromErp())
			{
				...
				System.out.println(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Before Delete Buss Partner");
				BussPartnerManager.deleteBussPartners(myEzcParams);
				System.out.println(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> After Delete Buss Partner");
%>
				<br><br><br><br>
...
%>
				...
				<Tr>
					...
					<img src="../../Images/Buttons/LAVENDER/back.gif" style="cursor:hand"  alt="Click Here To Back" border=no onClick="JavaScript:history.go(-1)">
				</center>
<%
				error=true;
				break;
