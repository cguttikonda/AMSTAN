...
function addhier()
{	
	document.myForm.action = "ezNewCustHierSync.jsp?BusinessPartner="+document.myForm.BusinessPartner.value;
...
<%
	...
	int textBoxCount = 0;
%>
<form name=myForm method=post action="ezCustReSync.jsp">
<%
...
<%   if ( numEzc > 0 ) { %>
		<input type="image" src="../../Images/Buttons/<%= ButtonDir%>/continue....gif"  onClick="checkAll();return document.returnValue">
<% } %>
		<img src="../../Images/Buttons/<%= ButtonDir%>/<%=btn%>.gif" onClick="addcust('<%=FUNCTION%>')" style="cursor:hand"  border=none>
		
		<img src="../../Images/Buttons/<%= ButtonDir%>/process.gif" onClick="addhier()" style="cursor:hand"  border=none>
