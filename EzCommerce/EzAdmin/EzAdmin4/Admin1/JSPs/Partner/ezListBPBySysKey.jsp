...
<%@ include file="../../../Includes/Lib/AddButtonDir.jsp"%>
	...
	<script>
		...
		function userDetails(partnerNo,sysKey)
		{
			var url = "ezUserDetails.jsp?partnerNo="+partnerNo+"&Area=<%=areaFlag%>&WebSysKey="+sysKey;
			
			var hWnd = 	window.open(url,"UserWindow","width=500,height=350,left=300,top=200,resizable=yes,scrollbars=yes,toolbar=no,menubar=no");
...
<%
		String isSel ="";
		String sysKeyTemp =websyskey; 
		String tabHeader = "<Th width='5%'>M</Th><Th width='15%'>Partner</Th><Th WIDTH='25%'>Description</Th><Th WIDTH='10%'>User</Th>";
		if("All".equals(websys)){
			isSel = "selected";
			tabHeader = "<Th width='5%'>M</Th><Th width='15%'>Partner</Th><Th WIDTH='25%'>Description</Th><Th WIDTH='20%'>Pur. Area</Th><Th WIDTH='25%'>Pur. Area Desc</Th><Th WIDTH='10%'>User</Th>";
		}	
		if(ret.getRowCount()>0)
		{
%>
...
%>
						...
						<label for="cb_<%=j%>">
							...
							<Td width="15%"><a href = "ezShowBPInfo.jsp?BusPartner=<%=ret1.getFieldValueString(j,"EBPC_BUSS_PARTNER")%>" class=bb><%=ret1.getFieldValueString(j,"EBPC_BUSS_PARTNER")%></a></Td>
							<Td width="25%"><%=ret1.getFieldValueString(j,"ECA_COMPANY_NAME")%></Td>
							<input type ="hidden" value="<%=ret1.getFieldValueString(j,"EBPA_SYS_KEY")%>+"/"+<%=ret1.getFieldValueString(j,"ESKD_SYS_KEY_DESC")%>">
<%
							if("All".equals(websys)){
							sysKeyTemp = ret1.getFieldValueString(j,"EBPA_SYS_KEY");
%>
								<Td width="20%"><%=ret1.getFieldValueString(j,"EBPA_SYS_KEY")%></Td>
...
%>
							...
							<Td width="10%" align="center">&nbsp;<a href="Javascript:userDetails('<%=ret1.getFieldValueString(j,"EBPC_BUSS_PARTNER")%>','<%=sysKeyTemp%>')">Details</a></Td>
						</label>
						</Tr>
