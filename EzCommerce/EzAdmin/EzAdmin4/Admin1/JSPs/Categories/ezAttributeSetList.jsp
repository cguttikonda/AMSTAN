...
<script type="text/javascript">
	...
	function funEdit()
	{
		var chkObj=document.myForm.attrID;
		var Count=0;
		if(!isNaN(chkObj.length))
		{
			for(i=0;i<chkObj.length;i++)
		...
		if(Count==1)
		{
			document.myForm.action="ezEditAttributeSet.jsp";
			document.myForm.submit();
		}		
	}
	function funDelete()
	{
		...
		if(Count>=1)
		{
			...
			if (r==true)
			  {
				document.myForm.action="ezDeleteAttributeSet.jsp";
				document.myForm.submit();
			  }
			else
			{
...
%>
		...
		<div class="info-box"><br>
			...
			<thead>
			<tr>
				<th width=5% align="center">&nbsp;&nbsp;&nbsp;<Input type="checkbox" name="attributeId" onclick="funCheckBox()"></th>
				<th width=20%>Attribute Id</th>
...
<%
			...
			{
				aStatus=retAttribueList.getFieldValueString(i,"EAS_STATUS");
				
				if(aStatus.equals("A"))aStatus="Active";
				if(aStatus.equals("I"))aStatus="Inactive";
				if(aStatus.equals("D"))aStatus="Deleted";
						
				
%>	
				<tr>
					<td width=5% align="center"><Input type="checkbox" name="attrID" value="<%=retAttribueList.getFieldValueString(i,"EAS_ID")%>¥<%=retAttribueList.getFieldValueString(i,"EAS_STATUS")%>¥<%=retAttribueList.getFieldValueString(i,"EAS_DESC")%>"></td>
					<td width=20%>&nbsp;<%=retAttribueList.getFieldValueString(i,"EAS_ID")%></td>
					<td width=5%>&nbsp;<%=aStatus%></td>
					<td width=25%>&nbsp;<%=retAttribueList.getFieldValueString(i,"EAS_DESC")%></td>
...
%>
	...
	<div align="center" style="position:relative">
		...
		<a href="ezAddAttributeSet.jsp"><img src="../../Images/Buttons/<%= ButtonDir%>/add.gif" border=none></a>	
